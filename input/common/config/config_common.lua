...
function ConfigCommon.ground_coxmunk_scaled:register_output(ro)
   ro:push_back(GroundCoxmunkScaledOutput.create(self.config.coxmunk, self.config.coxmunk_scaled, self.config.common.hdf_band_name))
end
...
function ConfigCommon:create_error_analysis()
   ...
   have_ch4 = self.absorber:gas_index("CH4") ~= -1
   ...
   have_co = self.absorber:gas_index("CO") ~= -1
   ...
   local out = ErrorAnalysisOutput(self.error_analysis, self:spec_flag(), 
				   have_co2, have_ch4, ch4_profile, have_co,
                                   co_profile)
