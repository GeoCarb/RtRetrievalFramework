...
!  everything public, except the Fourier routine
   ...
   PUBLIC  :: LIDORT_LPS_MASTER
   PRIVATE :: LIDORT_LPS_FOURIER
...
!  Version 3.8: LIDORT module dependencies
   ...
   USE lidort_lp_miscsetups_m       ! Need 2 public routines

   USE lidort_bvproblem_m,  only : BVP_MATRIXSETUP_MASTER,    BVP_SOLUTION_MASTER, &
                                   BVPTEL_MATRIXSETUP_MASTER, BVPTEL_SOLUTION_MASTER

   USE lidort_intensity_m   , only : LIDORT_CONVERGE,    LIDORT_CONVERGE_OBSGEO
   USE lidort_lp_wfatmos_m  , only : LIDORT_LP_CONVERGE, LIDORT_LP_CONVERGE_OBSGEO
   USE lidort_ls_wfsurface_m, only : LIDORT_LS_CONVERGE, LIDORT_LS_CONVERGE_OBSGEO
...
!   USE lidort_lp_corrections, only : LIDORT_LP_SSCORR_NADIR, LIDORT_LP_SSCORR_OUTGOING, LIDORT_LAP_DBCORRECTION
...
!  Profile weighting functions
      ...
      REAL(fpk) :: PROFILEWF ( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAX_GEOMETRIES, MAX_DIRECTIONS )

!  Mean intensity (actinic flux), Regular Flux, Diffuse values

      REAL(fpk) :: MEANI_DIFFUSE_PROFWF( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAXBEAMS, MAX_DIRECTIONS )
      REAL(fpk) :: FLUX_DIFFUSE_PROFWF ( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAXBEAMS, MAX_DIRECTIONS )

!  Direct-beam contributions output separately, added 7/18/17

      REAL(fpk) :: DNMEANI_DIRECT_PROFWF( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAXBEAMS )
      REAL(fpk) :: DNFLUX_DIRECT_PROFWF ( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAXBEAMS )

!  4/26-29/19. Special Media-property output. -- Introduced by R. Spurr.
!     ** Linearized Output developed for profile Jacobians.

      REAL(fpk) :: LP_ALBMED_USER  ( MAX_USER_STREAMS, MAXLAYERS, MAX_ATMOSWFS ) !  TOA illumination
      REAL(fpk) :: LP_TRNMED_USER  ( MAX_USER_STREAMS, MAXLAYERS, MAX_ATMOSWFS ) !  BOA illumination
      
      REAL(fpk) :: LP_ALBMED_FLUXES ( 2, MAXLAYERS, MAX_ATMOSWFS )    !  TOA illumination
      REAL(fpk) :: LP_TRNMED_FLUXES ( 2, MAXLAYERS, MAX_ATMOSWFS )    !  BOA illumination
      
      real(fpk) :: LP_TRANSBEAM ( MAXBEAMS, MAXLAYERS, MAX_ATMOSWFS ) !  Planetary problem
...
!  Solar
      ...
      real(fpk)  :: fo_profilewf_ss ( max_atmoswfs,maxlayers,max_user_levels,max_geometries,max_directions )
      real(fpk)  :: fo_profilewf_db ( max_atmoswfs,maxlayers,max_user_levels,max_geometries )
      real(fpk)  :: fo_surfacewf_db ( max_surfacewfs,max_user_levels,max_geometries )

!  Thermal

      real(fpk)  :: fo_profilewf_dta ( max_atmoswfs,maxlayers,max_user_levels,max_geometries,max_directions )
      real(fpk)  :: fo_profilewf_dts ( max_atmoswfs,maxlayers,max_user_levels,max_geometries )
      real(fpk)  :: fo_surfacewf_dts ( max_surfacewfs,max_user_levels,max_geometries )

!  Composite

      real(fpk)  :: fo_profilewf_atmos ( max_atmoswfs,maxlayers,max_user_levels,max_geometries,max_directions )
      real(fpk)  :: fo_profilewf_surf  ( max_atmoswfs,maxlayers,max_user_levels,max_geometries )
      real(fpk)  :: fo_profilewf       ( max_atmoswfs,maxlayers,max_user_levels,max_geometries,max_directions )
...
!  Solar beam attenuation to BOA (required for exact DB calculation)
!      REAL(fpk)       :: L_BOA_ATTN(MAX_ATMOSWFS,MAX_GEOMETRIES)
...
!  Last layer to include Particular integral solution
      ...
      INTEGER         :: SOLARBEAM_CUTOFF(MAXBEAMS)
...
!mick fix 1/19/2018 - added PARTIALS_SOLARTRAN
...
!  Rob fix 4/9/19, 4/30/19 Add linearization of SOLARBEAM_BOATRANS, SOLARBEAM_ATRANS
      ...
      REAL(fpk)       :: LP_SOLARBEAM_BOATRANS ( MAXBEAMS, MAXLAYERS, MAX_ATMOSWFS )
      REAL(fpk)       :: LP_SOLARBEAM_ATRANS   ( MAXBEAMS, MAXLAYERS, MAX_ATMOSWFS )

!  Rob fix 7/18/17 Added Arguments
!mick fix 1/19/2018 - added LP_PARTIALS_SOLARTRANS

      REAL(fpk)       :: LP_LEVELS_SOLARTRANS ( 0:MAXLAYERS, MAXLAYERS, MAXBEAMS, MAX_ATMOSWFS )
      REAL(fpk)       :: LP_PARTIALS_SOLARTRANS ( MAX_PARTLAYERS, MAXLAYERS, MAXBEAMS, MAX_ATMOSWFS )
...
!  4/9/19. Additional output from the SFO Interface, for the sleave correction
      ...
      real(fpk) :: FO_LP_CUMTRANS ( max_user_levels, MAX_GEOMETRIES, MAXLAYERS, MAX_ATMOSWFS )
...
!    --  The LP Jacobian is an approximation.
!    --  Also depends on surface/sleave quantities, but these linearizations (LS/LSSL) are neglected
      ...
      REAL(fpk) :: LP_TRANS_ATMOS_FINAL   ( MAXBEAMS, MAXLAYERS, MAX_ATMOSWFS   )
...
!  Local quantities
      ...
      REAL(fpk) :: LP_SLTERM_LOCAL(MAX_ATMOSWFS), LS_SLTERM_LOCAL(MAX_SLEAVEWFS), TFACTOR
...
!  Local flags for media problem and Planetary problem control
      ...
      REAL(fpk) :: TRANS, LP_TRANS 

!  Help variables
!  ==============

!  local inclusion flags. 3/22/17, for the TRANSLFUX routine
...
!    * Observational Geometry inputs introduced 10/25/12
      ...
      USER_OBSGEOMS(1:N_USER_OBSGEOMS,1:3)  = LIDORT_ModIn%MUserVal%TS_USER_OBSGEOM_INPUT(1:N_USER_OBSGEOMS,1:3)
...
!                     of thermal being used in the Lambertian case
      ...
      IF ( DO_BRDF_SURFACE ) THEN
        BRDF_F_0(0:NMOMS,1:NSTREAMS,1:NBEAMS) = &
          LIDORT_Sup%BRDF%TS_BRDF_F_0(0:NMOMS,1:NSTREAMS,1:NBEAMS)
        BRDF_F(0:NMOMS,1:NSTREAMS,1:NSTREAMS) = &
          LIDORT_Sup%BRDF%TS_BRDF_F(0:NMOMS,1:NSTREAMS,1:NSTREAMS)
        IF ( DO_USER_STREAMS ) THEN
          ...
          USER_BRDF_F_0 (0:NMOMS,1:N_USER_STREAMS,1:NBEAMS)   = &
            LIDORT_Sup%BRDF%TS_USER_BRDF_F_0(0:NMOMS,1:N_USER_STREAMS,1:NBEAMS)
          USER_BRDF_F   (0:NMOMS,1:N_USER_STREAMS,1:NSTREAMS)   = &
            LIDORT_Sup%BRDF%TS_USER_BRDF_F(0:NMOMS,1:N_USER_STREAMS,1:NSTREAMS)
        ENDIF
      ENDIF
...
!mick mod 3/22/2017 - added if conditions
      ...
      IF ( DO_SURFACE_LEAVING ) THEN
        ...
        SLTERM_F_0(0:NMOMS,1:NSTREAMS,1:NBEAMS) = &
          LIDORT_Sup%SLEAVE%TS_SLTERM_F_0(0:NMOMS,1:NSTREAMS,1:NBEAMS)
        IF ( DO_USER_STREAMS ) THEN
          ...
          USER_SLTERM_F_0(0:NMOMS,1:N_USER_STREAMS,1:NBEAMS) = &
            LIDORT_Sup%SLEAVE%TS_USER_SLTERM_F_0(0:NMOMS,1:N_USER_STREAMS,1:NBEAMS)
        ENDIF
      ENDIF
...
!  Local value of number of total profile WFs
      ...
      DO N = 1, NLAYERS
        N_TOTALPROFILE_WFS  = MAX(N_TOTALPROFILE_WFS,LAYER_VARY_NUMBER(N))
      ENDDO
...
!                     of thermal being used in the Lambertian case
      ...
      IF ( DO_BRDF_SURFACE .AND. DO_SURFACE_LINEARIZATION ) THEN
        LS_BRDF_F_0(1:N_SURFACE_WFS,0:NMOMS,1:NSTREAMS,1:NBEAMS) = &
          LIDORT_LinSup%BRDF%TS_LS_BRDF_F_0(1:N_SURFACE_WFS,0:NMOMS,1:NSTREAMS,1:NBEAMS)
        LS_BRDF_F(1:N_SURFACE_WFS,0:NMOMS,1:NSTREAMS,1:NSTREAMS) = &
          LIDORT_LinSup%BRDF%TS_LS_BRDF_F(1:N_SURFACE_WFS,0:NMOMS,1:NSTREAMS,1:NSTREAMS)
        IF ( DO_USER_STREAMS ) THEN
          ...
          LS_USER_BRDF_F_0(1:N_SURFACE_WFS,0:NMOMS,1:N_USER_STREAMS,1:NBEAMS) = &
            LIDORT_LinSup%BRDF%TS_LS_USER_BRDF_F_0(1:N_SURFACE_WFS,0:NMOMS,1:N_USER_STREAMS,1:NBEAMS)
          LS_USER_BRDF_F(1:N_SURFACE_WFS,0:NMOMS,1:N_USER_STREAMS,1:NSTREAMS) = &
            LIDORT_LinSup%BRDF%TS_LS_USER_BRDF_F(1:N_SURFACE_WFS,0:NMOMS,1:N_USER_STREAMS,1:NSTREAMS)
        ENDIF
      ENDIF
...
!mick mod 3/22/2017 - added if conditions
      ...
      IF ( DO_SURFACE_LEAVING .AND. DO_SLEAVE_WFS ) THEN
        ...
        LSSL_SLTERM_F_0(1:N_SLEAVE_WFS,0:NMOMS,1:NSTREAMS,1:NBEAMS)  = &
          LIDORT_LinSup%SLEAVE%TS_LSSL_SLTERM_F_0(1:N_SLEAVE_WFS,0:NMOMS,1:NSTREAMS,1:NBEAMS)
        IF ( DO_USER_STREAMS ) THEN
          ...
          LSSL_USER_SLTERM_F_0(1:N_SLEAVE_WFS,0:NMOMS,1:N_USER_STREAMS,1:NBEAMS) = &
            LIDORT_LinSup%SLEAVE%TS_LSSL_USER_SLTERM_F_0(1:N_SLEAVE_WFS,0:NMOMS,1:N_USER_STREAMS,1:NBEAMS)
        ENDIF
      ENDIF
...
!  Main linearized outputs (5 profile WF arrays, 3 surface WF arrays)

      PROFILEWF(1:MAX_ATMOSWFS,1:NLAYERS,1:N_USER_LEVELS,:,:) = ZERO
      MEANI_DIFFUSE_PROFWF (1:MAX_ATMOSWFS,1:NLAYERS,1:N_USER_LEVELS,1:NBEAMS,:)  = ZERO
      FLUX_DIFFUSE_PROFWF  (1:MAX_ATMOSWFS,1:NLAYERS,1:N_USER_LEVELS,1:NBEAMS,:)  = ZERO
      DNMEANI_DIRECT_PROFWF(1:MAX_ATMOSWFS,1:NLAYERS,1:N_USER_LEVELS,1:NBEAMS)    = ZERO
      DNFLUX_DIRECT_PROFWF (1:MAX_ATMOSWFS,1:NLAYERS,1:N_USER_LEVELS,1:NBEAMS)    = ZERO
...
!     ** Output for User-angle streams, also fluxes. TRANSBEAM for the planetary problem.
      ...
      LP_ALBMED_USER(1:N_USER_STREAMS,1:NLAYERS,1:MAX_ATMOSWFS) = zero
      LP_TRNMED_USER(1:N_USER_STREAMS,1:NLAYERS,1:MAX_ATMOSWFS) = zero

      LP_ALBMED_FLUXES(1:2,1:NLAYERS,1:MAX_ATMOSWFS) = zero
      LP_TRNMED_FLUXES(1:2,1:NLAYERS,1:MAX_ATMOSWFS) = zero

      LP_TRANSBEAM(1:NBEAMS,1:NLAYERS,1:MAX_ATMOSWFS) = zero

!  4/28/19. Initialize the planetary problem outputs

      LIDORT_LinOut%Atmos%TS_PLANETARY_SBTERM_PROFWF    (  1:NLAYERS,1:MAX_ATMOSWFS) = ZERO
      LIDORT_LinOut%Atmos%TS_PLANETARY_TRANSTERM_PROFWF (:,1:NLAYERS,1:MAX_ATMOSWFS) = ZERO

!  SS inputs
!  ---------

!  New 12 March 2012 --> IF SS results already available copy them !
!     Modified flagging, Version 3.8
...
!  Linearized arrays. No column stuff.

      COLUMNWF_SS = ZERO ; COLUMNWF_DB = ZERO
      IF ( DO_FOCORR_EXTERNAL ) THEN
...
!  Add Flag DO_WLADJUSTED_OUTPUT (Water-leaving output). 4/22/19 for Version 3.8.1. 
      ...
      ( DO_UPWELLING, DO_DNWELLING, DO_SOLAR_SOURCES, DO_THERMAL_EMISSION,           & ! Input
        ...
        DO_FOCORR, DO_OBSERVATION_GEOMETRY, DO_USER_STREAMS, DO_DELTAM_SCALING,      & ! Input, possibly modified
        DO_FOCORR_EXTERNAL, DO_FOCORR_ALONE, DO_FOCORR_NADIR, DO_FOCORR_OUTGOING,    & ! Input, possibly modified
        DO_REFRACTIVE_GEOMETRY, DO_CHAPMAN_FUNCTION, DO_PLANE_PARALLEL,              & ! Input, possibly modified
        DO_MVOUT_ONLY, DO_ADDITIONAL_MVOUT, DO_SOLUTION_SAVING, DO_BVP_TELESCOPING,  & ! Input, possibly modified
        TAYLOR_ORDER, NSTREAMS, N_USER_LEVELS, NLAYERS, TF_MAXITER,              & ! Fixed Inputs (line cleaned up)
...
!mick fix 3/22/2017 - added DO_FOCORR_NADIR & DO_FOCORR_OUTGOING to input
      ...
      CALL LIDORT_DERIVE_INPUT ( &
        ...
        DO_FOCORR_NADIR, DO_FOCORR_OUTGOING,                                   & ! Input Boolean
        DO_REFRACTIVE_GEOMETRY, DO_RAYLEIGH_ONLY, DO_ISOTROPIC_ONLY,           & ! Input Boolean
        DO_UPWELLING, DO_DNWELLING, DO_USER_STREAMS, DO_OBSERVATION_GEOMETRY,  & ! Input Boolean
...
!   4/30/19 Add SOLARBEAM_ATRANS for LP_SOLARBEAM_ATRANS output
      ...
      IF ( DO_PROFILE_LINEARIZATION ) THEN
        ...
        ( DO_SOLAR_SOURCES, DO_PLANE_PARALLEL,                    & ! Input
          ...
          LP_AVERAGE_SECANT, LP_SOLARBEAM_ATRANS, HELP_AQ, HELP_BQ )! Output
      ENDIF

!  Rob Fix, 7/18/17. level solar trans for all, Profile Linearization
        ...
        DO IB = 1, NBEAMS
          do N = 1, NLAYERS
           IF ( LEVELS_SOLARTRANS(N,IB) .gt. zero  ) THEN
            ...
            IF ( N==NLAYERS ) THEN
              DO K = 1, N
                LP_SOLARBEAM_BOATRANS  (IB,K,1:LAYER_VARY_NUMBER(K)) = &
                              LP_LEVELS_SOLARTRANS  (N,K,IB,1:LAYER_VARY_NUMBER(K))
              ENDDO
            ENDIF
          enddo
        ENDDO
...
!  Partial layers
      ...
      IF ( DO_PROFILE_LINEARIZATION .and. N_PARTLAYERS .gt. 0 ) THEN
        DO IB = 1, NBEAMS
      ...
      IF ( DO_SOLAR_SOURCES .AND. DO_USER_STREAMS) THEN
       ...
       ( DO_UPWELLING, DO_DNWELLING, DO_OBSERVATION_GEOMETRY,     & ! Input
         ...
         ( DO_UPWELLING, DO_DNWELLING, DO_OBSERVATION_GEOMETRY,     & ! Input
           ...
           DELTAU_VERT, PARTAU_VERT, L_DELTAU_VERT,                 & ! input
           T_DELT_USERM, T_UTUP_USERM, T_UTDN_USERM,                & ! input
           ITRANS_USERM, SOLARBEAM_CUTOFF,  T_DELT_MUBAR,           & ! input
...
!  @@@@@@@@@@@ Robfix 19 March 2012.   Do the Regular setups if no PROFILE linearization

      IF ( DO_THERMAL_EMISSION ) THEN
        IF ( DO_PROFILE_LINEARIZATION ) THEN
          CALL THERMAL_SETUP_PLUS ( &
            DO_USER_STREAMS, DO_UPWELLING, DO_DNWELLING, DO_PARTLAYERS,       & ! Flags
            DO_THERMAL_TRANSONLY, DO_MSMODE_THERMAL, DO_PROFILE_LINEARIZATION, & ! Flags
...
!            SOLARBEAM_CUTOFF, T_DELT_MUBAR, INITIAL_TRANS, AVERAGE_SECANT,      & ! Input solar beam
!            TRANS_ATMOS_FINAL, FLUX_FINAL,                                      & ! Output
!            STATUS_SUB, MESSAGE, TRACE_1, TRACE_2 )                               ! Output
!            IF ( STATUS_SUB == LIDORT_SERIOUS ) THEN
!               TRACE_3 = 'LIDORT_TRANSFLUX_MASTER failed, LIDORT_LPS_MASTER'
!               STATUS_CALCULATION = LIDORT_SERIOUS
!               LIDORT_Out%Status%TS_STATUS_CALCULATION = STATUS_CALCULATION
!               LIDORT_Out%Status%TS_MESSAGE = MESSAGE
!               LIDORT_Out%Status%TS_TRACE_1 = TRACE_1
...
!           DO IBEAM = 1, NBEAMS
!             LSSL_SLTERM_ISOTROPIC(1:N_SLEAVE_WFS,IBEAM) =  LSSL_SLTERM_ISOTROPIC(1:N_SLEAVE_WFS,IBEAM) &
!                                                          * TRANS_ATMOS_FINAL(IBEAM)
!           ENDDO
!         ENDIF
...
!              Added input, water-leaving control
            ...
            CALL SFO_LPS_MASTER_INTERFACE ( &
              ...
              DO_SSCORR_USEPHASFUNC, DO_BRDF_SURFACE, DO_SURFACE_LEAVING, DO_WATER_LEAVING,       & ! Input Optical/Surface flags
              DO_SL_ISOTROPIC, DO_PROFILE_LINEARIZATION, DO_SURFACE_LINEARIZATION, DO_SLEAVE_WFS, & ! Input Jacobian flags
              NLAYERS, NFINELAYERS, NMOMENTS_INPUT, NBEAMS, BEAM_SZAS,                            & ! Input numbers
              N_USER_STREAMS, USER_ANGLES, N_USER_RELAZMS, USER_RELAZMS,                          & ! Input geometry
              N_USER_LEVELS, UTAU_LEVEL_MASK_UP, UTAU_LEVEL_MASK_DN, N_PARTLAYERS,             & ! Input levels  control
              LAYER_VARY_FLAG, LAYER_VARY_NUMBER, N_SLEAVE_WFS, N_SURFACE_WFS, N_TOTALSURFACE_WFS, & ! Input numbers (Jacobians)
              ...
              FO_PROFILEWF_ATMOS,  FO_PROFILEWF_SURF,  FO_PROFILEWF, FO_SURFACEWF,     & ! Output compiled Jacobians
              FO_CUMTRANS, FO_SLTERM, FO_LP_CUMTRANS, FO_LSSL_SLTERM,                  & ! Output Auxiliary 4/9/19 New line
              FAIL, MESSAGE, TRACE_1, TRACE_2 )                                          ! Output
       
!  Exception handling

            IF ( FAIL ) THEN
               TRACE_3 = 'SFO_LPS_MASTER_INTERFACE failed, LIDORT_LPS_MASTER'
               STATUS_CALCULATION = LIDORT_SERIOUS
               LIDORT_Out%Status%TS_STATUS_CALCULATION = STATUS_CALCULATION
               LIDORT_Out%Status%TS_MESSAGE = MESSAGE
               LIDORT_Out%Status%TS_TRACE_1 = TRACE_1
...
!mick mod 3/22/2017  - added if conditions
!mick note 3/22/2017 - Important!  FO_INTENSITY_SURF, FO_PROFILEWF_SURF, &
...
!                      terms when computing in the crossover region!
            ...
            INTENSITY_SS(1:N_USER_LEVELS,1:N_GEOMETRIES,:) = &
               FO_INTENSITY_ATMOS(1:N_USER_LEVELS,1:N_GEOMETRIES,:)
            INTENSITY_DB(1:N_USER_LEVELS,1:N_GEOMETRIES)   = &
               FO_INTENSITY_SURF (1:N_USER_LEVELS,1:N_GEOMETRIES)

            IF ( DO_PROFILE_LINEARIZATION ) THEN
              PROFILEWF_SS(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES,:) = &
                FO_PROFILEWF_ATMOS(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES,:)
              PROFILEWF_DB(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES)   = &
                FO_PROFILEWF_SURF(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES)
            ENDIF

            IF ( DO_SURFACE_LINEARIZATION ) THEN
              !SURFACEWF_DB(1:N_SURFACE_WFS,1:N_USER_LEVELS,1:N_GEOMETRIES) = &
              !  FO_SURFACEWF_DB(1:N_SURFACE_WFS,1:N_USER_LEVELS,1:N_GEOMETRIES)
...
!    -- Associated outputs are TRANSBEAM, ALBMED_USER, ALBMED_FLUXES, TRNMED_USER, TRNMED_FLUXES   
           ...
           CALL LIDORT_LPS_FOURIER ( FOURIER, &
            ...
            DO_PLANE_PARALLEL,DO_LAYER_SCATTERING,DO_SOLUTION_SAVING,DO_BVP_TELESCOPING,DO_BVTEL_INITIAL,& !Input flags(performance)
            ...
            DO_PROFILE_LINEARIZATION, DO_SURFACE_LINEARIZATION, DO_SLEAVE_WFS,                      & !Input flags Linearization
            DO_ATMOS_LBBF, DO_SURFACE_LBBF,                                                         & !Input flags LBBF
            NSTREAMS, NLAYERS, NBEAMS, N_USER_STREAMS, N_USER_LEVELS, N_THERMAL_COEFFS, NMOMENTS,   & !Input Control Numbers
            NSTREAMS_2, NTOTAL, N_SUBDIAG, N_SUPDIAG, N_PARTLAYERS, N_DIRECTIONS, WHICH_DIRECTIONS, & !Input Bookkeeping
            N_TOTALPROFILE_WFS, LAYER_VARY_FLAG, LAYER_VARY_NUMBER, N_SURFACE_WFS, N_SLEAVE_WFS,    & !Input Linearization
            N_PPSTREAMS, PPSTREAM_MASK, TAYLOR_ORDER, FLUX_FACTOR, BEAM_COSINES, SUNLAYER_COSINES,  & !Input (Mask, SZA and flux)
            LOCAL_CSZA, USER_STREAMS, USER_SECANTS, QUAD_STREAMS, QUAD_WEIGHTS, QUAD_STRMWTS,       & !Input (Streams)
            ...
            ALBEDO, BRDF_F, BRDF_F_0, USER_BRDF_F, USER_BRDF_F_0,                                   & !Input (Surface Refl.)
            SLTERM_ISOTROPIC, SLTERM_F_0, USER_SLTERM_F_0,                                          & !Input (Surface Leaving)
            SURFBB, EMISSIVITY, USER_EMISSIVITY,                                                    & !Input (Surface thermal)
            ...
            INITIAL_TRANS, AVERAGE_SECANT, T_DELT_MUBAR, T_UTDN_MUBAR, ITRANS_USERM, SOLARBEAM_CUTOFF, & !Input (Beam AvSec)
            SOLARBEAM_BOATRANS, SOLARBEAM_ATRANS, LEVELS_SOLARTRANS, PARTIALS_SOLARTRANS,              & !Input (Beam Trans)
            T_DELT_USERM, T_UTUP_USERM, T_UTDN_USERM,                                               & !Input (User Trans)
            T_DELT_DISORDS, T_DISORDS_UTUP, T_DISORDS_UTDN,                                         & !Input (Dom  Trans)
            EMULT_UP, EMULT_DN, UT_EMULT_UP, UT_EMULT_DN, SIGMA_P, SIGMA_M,                         & !Input (Beam Mult.)
            LS_BRDF_F_0, LS_BRDF_F, LS_USER_BRDF_F_0, LS_USER_BRDF_F, LS_EMISSIVITY, LS_USER_EMISSIVITY, & ! Input LS   BRDFs
            LSSL_SLTERM_ISOTROPIC, LSSL_SLTERM_USERANGLES, LSSL_SLTERM_F_0, LSSL_USER_SLTERM_F_0,        & ! Input LSSL Sleave
            ...
            LP_INITIAL_TRANS, LP_AVERAGE_SECANT, LP_T_DELT_MUBAR, LP_T_UTDN_MUBAR,                       & ! Input LP Solar
            LP_SOLARBEAM_BOATRANS, LP_SOLARBEAM_ATRANS, LP_LEVELS_SOLARTRANS, LP_PARTIALS_SOLARTRANS,    & ! Input LP Solar
            ...
            LP_EMULT_UP, LP_EMULT_DN, LP_UT_EMULT_UP, LP_UT_EMULT_DN, HELP_AQ, HELP_BQ,                  & ! Input LP Trans.
            INTENSITY_F, MEANI_DIFFUSE, FLUX_DIFFUSE, DNMEANI_DIRECT, DNFLUX_DIRECT,                             & !Output Int/Flux
            PROFILEWF_F, MEANI_DIFFUSE_PROFWF, FLUX_DIFFUSE_PROFWF, DNMEANI_DIRECT_PROFWF, DNFLUX_DIRECT_PROFWF, & !Output LP Jacs
            SURFACEWF_F, MEANI_DIFFUSE_SURFWF, FLUX_DIFFUSE_SURFWF,                                              & !Output LS Jacs
            ...
            LP_TRANS_ATMOS_FINAL, LP_TRANSBEAM, LP_ALBMED_USER, LP_ALBMED_FLUXES, LP_TRNMED_USER, LP_TRNMED_FLUXES,& !Output 4/26/19
            ...
            if ( do_OBSERVATION_GEOMETRY ) THEN
              DO IB = 1, NBEAMS
                ...
                do uta = 1, n_user_levels
                  ...
                  INTENSITY_DB(UTA,IB) = INTENSITY_DB(UTA,IB) + CUMSOURCE_DB
                enddo
                if ( do_profile_linearization ) then
                  DO K = 1, nlayers
                    if ( layer_vary_flag(k) ) then
                      NWFS = layer_vary_number(k)
                      LP_SLTERM_LOCAL(1:NWFS) = FO_SLTERM(ib) * LP_TRANS_ATMOS_FINAL(ib,k,1:NWFS) 
                      do uta = 1, n_user_levels
                        DO Q = 1, NWFS
                          L_CUMSOURCE_DB =    FO_CUMTRANS(UTA,IB)     * LP_SLTERM_LOCAL(Q) &
                                         + FO_LP_CUMTRANS(UTA,IB,K,Q) *    SLTERM_LOCAL
                          PROFILEWF_DB(Q,K,UTA,IB) = PROFILEWF_DB(Q,K,UTA,IB) + L_CUMSOURCE_DB
                        enddo
                      enddo
                    endif
                  enddo
                ...
                if ( do_water_leaving .and. DO_SLEAVE_WFS ) then
                  ...
                  do uta = 1, n_user_levels
                    DO Q = 1, NWFS
                      ...
                      SURFACEWF_DB(Q,UTA,IB) = SURFACEWF_DB(Q,UTA,IB) + LS_CUMSOURCE_DB
                    enddo
                  enddo
                endif
              enddo
              ...
              DO IB = 1, NBEAMS ; DO UM = 1, n_user_streams
                ...
                do uta = 1, n_user_levels
                  do g = g1, g2
                    ...
                    INTENSITY_DB(UTA,g) =INTENSITY_DB(UTA,g) + CUMSOURCE_DB
                  enddo
                enddo
                if ( do_profile_linearization ) then
                  DO K = 1, nlayers
                    if ( layer_vary_flag(k) ) then
                      NWFS = layer_vary_number(k)
                      LP_SLTERM_LOCAL(1:NWFS) = FO_SLTERM(g1) * LP_TRANS_ATMOS_FINAL(ib,k,1:NWFS) 
                      do uta = 1, n_user_levels
                        do g = g1, g2
                          DO Q = 1, NWFS
                            L_CUMSOURCE_DB =    FO_CUMTRANS(UTA,G)   * LP_SLTERM_LOCAL(Q) &
                                              + FO_LP_CUMTRANS(UTA,G,K,Q) *    SLTERM_LOCAL
                            PROFILEWF_DB(Q,K,UTA,G) = PROFILEWF_DB(Q,K,UTA,G) + L_CUMSOURCE_DB
                          enddo
                        enddo
                      enddo
                    endif
                ...
                if ( do_water_leaving .and. DO_SLEAVE_WFS ) then
                  ...
                  do uta = 1, n_user_levels
                    DO Q = 1, NWFS
                      LS_CUMSOURCE_DB =  LS_SLTERM_LOCAL(Q) ; Q1 = Q + N_SURFACE_WFS
                      SURFACEWF_DB(Q,UTA,G) = SURFACEWF_DB(Q,UTA,G) + LS_CUMSOURCE_DB
                    enddo
                  enddo
                endif
              enddo ; enddo
      ...
      ! Debug results - USE WITH CAUTION. Note the preliminary zeroing to avoid unassigned arrays.
          ...
          if ( FOURIER .eq. 0 .and. DO_WATER_LEAVING .and. DO_WLADJUSTED_OUTPUT ) then
             ...
             DO IB = 1, NBEAMS
                ...
                LIDORT_Out%WLOut%TS_WLADJUSTED_F_Ords_0(0,1:NSTREAMS,IB) = TFACTOR * SLTERM_F_0(0,1:NSTREAMS,IB)
                IF ( DO_USER_STREAMS ) THEN
                   ...
                   LIDORT_Out%WLOut%TS_WLADJUSTED_F_User_0(0,1:N_USER_STREAMS,IB) = &
                                           TFACTOR * USER_SLTERM_F_0(0,1:N_USER_STREAMS,IB)
                ENDIF
             ENDDO
          ENDIF
...
!                     before call to LIDORT_LPS_FOURIER to here
          ...
          IF ( FOURIER .GT. 0 ) THEN
            ...
            IF ( .not. DO_OBSERVATION_GEOMETRY ) THEN
              DO UA = 1, LOCAL_N_USERAZM
                AZMFAC(1:N_USER_STREAMS,1:NBEAMS,UA) = DCOS(DEG_TO_RAD*DFC*USER_RELAZMS(UA))
              ENDDO
            ELSE
              DO IB = 1, NBEAMS
                AZMFAC(LUM,IB,LUA) = DCOS(DEG_TO_RAD*DFC*USER_RELAZMS(IB))
              ENDDO
            ENDIF
          ENDIF
...
!  Convergence and radiance summation
                ...
                CALL LIDORT_CONVERGE                                       &
                 ( DO_FOCORR, DO_FOCORR_ALONE, DO_UPWELLING, DO_NO_AZIMUTH,   & ! Input flags
                   ...
                   IBEAM, FOURIER, N_CONVTESTS, LIDORT_ACCURACY, VZA_OFFSETS, & ! Input numbers, convergence
                ...
                IF ( DO_PROFILE_LINEARIZATION ) THEN
                  CALL LIDORT_LP_CONVERGE  &
                   ( DO_FOCORR, DO_FOCORR_ALONE, DO_UPWELLING, DO_NO_AZIMUTH,     & ! Input flags
                     ...
                     IBEAM, FOURIER, NLAYERS, LAYER_VARY_FLAG, LAYER_VARY_NUMBER, & ! Input numbers
                     VZA_OFFSETS, N_DIRECTIONS, WHICH_DIRECTIONS,                 & ! Input bookkeeping
...
!  Fourier summation of Surface Jacobians
                ...
                IF ( DO_SURFACE_LINEARIZATION ) THEN
                  ...
                  ( DO_FOCORR, DO_FOCORR_ALONE,                                   & ! Input flags
                    ...
                    N_DIRECTIONS, WHICH_DIRECTIONS, VZA_OFFSETS, LOCAL_N_USERAZM, & ! Input bookkeeping
...
!  Fourier summation of Profile Jacobians
                ...
                IF ( DO_PROFILE_LINEARIZATION ) THEN
                  ...
                  ( DO_FOCORR, DO_FOCORR_ALONE, DO_UPWELLING, DO_NO_AZIMUTH, & ! Input flags
                    N_USER_LEVELS, IBEAM, FOURIER, NLAYERS, LAYER_VARY_FLAG, & ! Input numbers
                    LAYER_VARY_NUMBER, N_DIRECTIONS, WHICH_DIRECTIONS,       & ! Input bookkeeping
                    AZMFAC, PROFILEWF_F, PROFILEWF_SS, PROFILEWF_DB,         & ! Input Azm, fields
                    PROFILEWF )                                                ! Output
                ENDIF
...
!          IF ( DO_PROFILE_LINEARIZATION ) THEN
!            CALL LIDORT_LP_WRITEFOURIER ( DO_INCLUDE_SURFACE, FUNIT, FOURIER )
!          ENDIF
...
!  New Code, 26 October 2012
      ...
      LIDORT_ModIn%MUserVal%TS_USER_OBSGEOM_INPUT(1:N_USER_OBSGEOMS,:) = USER_OBSGEOMS(1:N_USER_OBSGEOMS,:)
...
!  Profile Jacobians
      ...
      LIDORT_LinOut%Atmos%TS_PROFILEWF(1:MAX_ATMOSWFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES,:)      = &
                             PROFILEWF(1:MAX_ATMOSWFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES,:)

      LIDORT_LinOut%Atmos%TS_MEANI_DIFFUSE_PROFWF(1:MAX_ATMOSWFS,1:NLAYERS,1:N_USER_LEVELS,1:NBEAMS,:) = &
                             MEANI_DIFFUSE_PROFWF(1:MAX_ATMOSWFS,1:NLAYERS,1:N_USER_LEVELS,1:NBEAMS,:)
      LIDORT_LinOut%Atmos%TS_FLUX_DIFFUSE_PROFWF (1:MAX_ATMOSWFS,1:NLAYERS,1:N_USER_LEVELS,1:NBEAMS,:) = &
                             FLUX_DIFFUSE_PROFWF (1:MAX_ATMOSWFS,1:NLAYERS,1:N_USER_LEVELS,1:NBEAMS,:)

      LIDORT_LinOut%Atmos%TS_DNMEANI_DIRECT_PROFWF(1:MAX_ATMOSWFS,1:NLAYERS,1:N_USER_LEVELS,1:NBEAMS)  = &
                             DNMEANI_DIRECT_PROFWF(1:MAX_ATMOSWFS,1:NLAYERS,1:N_USER_LEVELS,1:NBEAMS)
      LIDORT_LinOut%Atmos%TS_DNFLUX_DIRECT_PROFWF (1:MAX_ATMOSWFS,1:NLAYERS,1:N_USER_LEVELS,1:NBEAMS)  = &
                             DNFLUX_DIRECT_PROFWF (1:MAX_ATMOSWFS,1:NLAYERS,1:N_USER_LEVELS,1:NBEAMS)

!  4/26/19. Media properties output.

      LIDORT_LinOut%Atmos%TS_ALBMED_USER_PROFWF(1:N_USER_STREAMS,1:NLAYERS,:) = LP_ALBMED_USER(1:N_USER_STREAMS,1:NLAYERS,:)
      LIDORT_LinOut%Atmos%TS_TRNMED_USER_PROFWF(1:N_USER_STREAMS,1:NLAYERS,:) = LP_TRNMED_USER(1:N_USER_STREAMS,1:NLAYERS,:)
      LIDORT_LinOut%Atmos%TS_ALBMED_FLUXES_PROFWF(:,1:NLAYERS,:) = LP_ALBMED_FLUXES(:,1:NLAYERS,:)
      LIDORT_LinOut%Atmos%TS_TRNMED_FLUXES_PROFWF(:,1:NLAYERS,:) = LP_TRNMED_FLUXES(:,1:NLAYERS,:)

!  4/28/19. Planetary problem output
      
      IF ( DO_PLANETARY_PROBLEM .and. DO_PROFILE_LINEARIZATION ) THEN
         LIDORT_LinOut%Atmos%TS_PLANETARY_SBTERM_PROFWF(1:nlayers,:) = LP_TRNMED_FLUXES(2,1:nlayers,:)
         IF ( DO_OBSERVATION_GEOMETRY ) THEN
            DO IB =1, N_GEOMETRIES
               DO N = 1, NLAYERS
                  DO Q = 1, LAYER_VARY_NUMBER(N)
                     LIDORT_LinOut%Atmos%TS_PLANETARY_TRANSTERM_PROFWF(IB,N,Q) = &
                          ( LP_TRANSBEAM(IB,N,Q) * TRNMED_USER(IB) + TRANSBEAM(IB) * LP_TRNMED_USER(IB,N,Q) ) / PIE
                  ENDDO
               ENDDO
            ENDDO
         ELSE
            DO IB =1, NBEAMS ; DO UM = 1, N_USER_STREAMS
               ...
               DO N = 1, NLAYERS
                  DO Q = 1, LAYER_VARY_NUMBER(N)
                     L_TRANS(Q) =  ( LP_TRANSBEAM(IB,N,Q) * TRNMED_USER(UM) + TRANSBEAM(IB) * LP_TRNMED_USER(UM,N,Q) ) / PIE
                     LIDORT_LinOut%Atmos%TS_PLANETARY_TRANSTERM_PROFWF(OFF+1:OFF+N_USER_RELAZMS,N,Q) = L_TRANS(Q)
                  ENDDO
               ENDDO
            ENDDO ; ENDDO
         ENDIF   
...
!   IF SS results already available, no need to copy them !
      ...
      IF ( .NOT. DO_FOCORR_EXTERNAL ) THEN
         LIDORT_LinSup%SS%Atmos%TS_PROFILEWF_SS(1:MAX_ATMOSWFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES,:) = &
                                   PROFILEWF_SS(1:MAX_ATMOSWFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES,:)
         LIDORT_LinSup%SS%Atmos%TS_PROFILEWF_DB(1:MAX_ATMOSWFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES)   = &
...
!  4/22/19, Version 3.8.1, Record Control for TOA/BOA illumination
      ...
      CALL LIDORT_WRITE_STD_INPUT ( &
        ...
        DO_FULLRAD_MODE,    DO_FOCORR,           DO_FOCORR_EXTERNAL,                            & ! FOCORR/Planetary
        ...
        DO_USER_STREAMS,        DO_OBSERVATION_GEOMETRY, DO_ADDITIONAL_MVOUT, DO_MVOUT_ONLY,    & ! RT model
      ...
      IF (DO_BRDF_SURFACE .AND. DO_SURFACE_LINEARIZATION) THEN
        CALL LIDORT_WRITE_LIN_SUP_BRDF_INPUT ( &
          DO_USER_STREAMS, DO_SURFACE_EMISSION, &
          NBEAMS, N_USER_STREAMS, N_USER_RELAZMS, NSTREAMS, NMOMS, N_SURFACE_WFS, &
      ...
      IF (DO_SURFACE_LEAVING .AND. DO_SURFACE_LINEARIZATION .AND. DO_SLEAVE_WFS) THEN
        CALL LIDORT_WRITE_LIN_SUP_SLEAVE_INPUT ( &
          DO_USER_STREAMS, &
          NBEAMS, N_USER_STREAMS, N_USER_RELAZMS, NSTREAMS, NMOMS, N_SLEAVE_WFS, &
...
SUBROUTINE LIDORT_LPS_FOURIER ( FOURIER, &
            DO_UPWELLING, DO_DNWELLING, DO_USER_STREAMS, DO_OBSERVATION_GEOMETRY, DO_FOCORR,        & !Input flags (RT operation)
            DO_SOLAR_SOURCES, DO_REFRACTIVE_GEOMETRY, DO_ADDITIONAL_MVOUT, DO_MVOUT_ONLY,           & !Input flags (RT operation)
            DO_PLANE_PARALLEL,DO_LAYER_SCATTERING,DO_SOLUTION_SAVING,DO_BVP_TELESCOPING,DO_BVTEL_INITIAL,& !Input flags(performance)
...
!  Argument list revised for Version 3.8a, 4/26/19, 4/28/19
...
! -- Jacobian
   ...
   USE LIDORT_LP_BVPROBLEM_m, Only : LP_BVP_SOLUTION_MASTER, LP_BVPTEL_SOLUTION_MASTER
   USE LIDORT_LPC_SOLUTIONS_m
   USE LIDORT_LP_WFATMOS_m  , Only : UPUSER_PROFILEWF, DNUSER_PROFILEWF, MIFLUX_PROFILEWF, GET_LP_TOASOURCE,GET_LP_BOASOURCE
   USE LIDORT_LS_WFSURFACE_m, Only : SURFACEWF_BVP_SOLUTION, SURFACEWF_BVPTEL_SOLUTION, SURFACEWF_MASTER
...
!  Rob fix 7/18/17 Added Arguments
!mick fix 1/19/2018 - added LP_PARTIALS_SOLARTRANS
!Rob  fix 4/9/19,4/29/19    - added LP_SOLARBEAM_BOATRANS, LP_SOLARBEAM_ATRANS

      REAL(fpk), intent(in)  :: LP_SOLARBEAM_BOATRANS  ( MAXBEAMS, MAXLAYERS, MAX_ATMOSWFS )
      REAL(fpk), intent(in)  :: LP_SOLARBEAM_ATRANS    ( MAXBEAMS, MAXLAYERS, MAX_ATMOSWFS )
...
!    --  Also depends on surface/sleave quantities, but these linearizations (LS/LSSL) are neglected
      ...
      REAL(fpk), INTENT (INOUT) :: LP_TRANS_ATMOS_FINAL ( MAXBEAMS, MAXLAYERS, MAX_ATMOSWFS )

!  4/26-29/19. Special Media-property output. -- Introduced by R. Spurr.
!     ** Linearized Output developed for profile Jacobians.

      REAL(fpk), INTENT (INOUT) :: LP_ALBMED_USER  ( MAX_USER_STREAMS, MAXLAYERS, MAX_ATMOSWFS ) !  TOA illumination
      REAL(fpk), INTENT (INOUT) :: LP_TRNMED_USER  ( MAX_USER_STREAMS, MAXLAYERS, MAX_ATMOSWFS ) !  BOA illumination
      
      REAL(fpk), INTENT (INOUT) :: LP_ALBMED_FLUXES ( 2, MAXLAYERS, MAX_ATMOSWFS )    !  TOA illumination
      REAL(fpk), INTENT (INOUT) :: LP_TRNMED_FLUXES ( 2, MAXLAYERS, MAX_ATMOSWFS )    !  BOA illumination
      
      real(fpk), INTENT (INOUT) :: LP_TRANSBEAM ( MAXBEAMS, MAXLAYERS, MAX_ATMOSWFS ) !  Planetary problem
...
!  Profile weighting functions at quadrature angles
      ...
      REAL(fpk)  :: QUADPROFILEWF ( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAXSTREAMS, MAXBEAMS, MAX_DIRECTIONS )
...
!  Indices and Help
      ...
      INTEGER         :: LAYER, IBEAM, I, AA, N, Q, K, LUM, UM
      real(fpk)       :: TRANSQUAD(MAXSTREAMS), TRANS, DIRECT_FLUX
...
!  .. error tracing
            ...
            IF ( STATUS_SUB .NE. LIDORT_SUCCESS ) THEN
               write(CF,'(I2)')FOURIER
               TRACE_2 =  'Called in LIDORT_LPS_FOURIER, Fourier component '//CF
               TRACE_3 =  'LIDORT_LPS_FOURIER call '
...
!  4/26/19. Add Call to Media properties subroutines (regular or LP_Jacobian)
          ...
          IF ( DO_PROFILE_LINEARIZATION ) THEN
           CALL LIDORT_LP_MediaProps &
            ( DO_USER_STREAMS, DO_ALBTRN_MEDIA,                             & ! Input
              ...
              DO_PROFILE_LINEARIZATION, LAYER_VARY_FLAG, LAYER_VARY_NUMBER, & ! Input
              ...
              STATUS_SUB, MESSAGE, TRACE_1, TRACE_2 )                         ! Output
           IF ( STATUS_SUB .NE. LIDORT_SUCCESS ) THEN
             TRACE_3 = 'Error from LIDORT_LP_MediaProps, '//'Called in LIDORT_LPS_FOURIER'
             STATUS = LIDORT_SERIOUS ; RETURN
           ENDIF
         ELSE
           CALL LIDORT_MediaProps &
            ( DO_USER_STREAMS, DO_ALBTRN_MEDIA,                             & ! Input
              ...
              STATUS_SUB, MESSAGE, TRACE_1, TRACE_2 )                         ! Output
           IF ( STATUS_SUB .NE. LIDORT_SUCCESS ) THEN
             TRACE_3 = 'Error from LIDORT_MediaProps, '//'Called in LIDORT_LPS_FOURIER'
             STATUS = LIDORT_SERIOUS ; RETURN
           ENDIF
         ENDIF   
       ENDIF
...
!     I     omega_moms(1,0), L_omega_moms(1,1,0),     ! replaced line
        ...
        IF ( DO_PROFILE_LINEARIZATION ) THEN
          ...
          ( DO_UPWELLING, DO_DNWELLING, DO_THERMAL_TRANSONLY,    & ! Input
            ...
            DO_PROFILE_LINEARIZATION,                            & ! Input
...
!   Version 3.8.1, Control for TOA/BOA illumination added, 4/22/19
          ...
          CALL BVP_SOLUTION_MASTER ( & 
              ...
              SUNLAYER_COSINES, SOLARBEAM_CUTOFF, T_DELT_MUBAR, INITIAL_TRANS,    & ! Input Direct-flux
...
!  4/9/19. Use PPSTREAMS post-processing mask
           ...
           CALL GET_BOASOURCE &
            ( DO_USER_STREAMS, DO_INCLUDE_MVOUTPUT, DO_MSMODE_THERMAL,         & ! Input
              ...
              N_PPSTREAMS, PPSTREAM_MASK, NSTREAMS, NLAYERS, IBEAM, FOURIER,   & ! Input
...
!  4/9/19. Use PPSTREAMS post-processing mask
          ...
          CALL UPUSER_INTENSITY &
            ( DO_USER_STREAMS, DO_SOLAR_SOURCES, DO_MSMODE_LIDORT,            & ! Input
              DO_THERMAL_EMISSION, DO_THERMAL_TRANSONLY, DO_INCLUDE_MVOUTPUT, & ! Input
...
!  4/9/19. Use PPSTREAMS post-processing mask
           ...
           CALL GET_TOASOURCE ( N_PPSTREAMS, PPSTREAM_MASK, IBEAM, TOA_SOURCE )
...
!     Here in the thermal-regime, there is no water-leaving, so initialize to zero          
            ...
            LP_TRANS_ATMOS_FINAL(IBEAM,LAYER_TO_VARY,1:N_LAYER_WFS) = zero
...
!  4/9/19. Water-leaving control. Also need LC_TRANS_ATMOS_FINAL
            ...
            CALL LP_BVP_SOLUTION_MASTER &
             ( DO_INCLUDE_SURFACE, DO_BRDF_SURFACE, DO_INCLUDE_DIRECTRF,       & ! Input
               ...
               LP_TRANS_ATMOS_FINAL, L_KEIGEN, L_T_DELT_EIGEN, L_XPOS, L_XNEG, & ! Input, Linearized Homogeneous  
             ...
             IF ( STATUS_SUB .NE. LIDORT_SUCCESS ) THEN
               write(CF,'(I2)')FOURIER
               TRACE_2 = 'Error return from LP_BVP_SOLUTION_MASTER, thermal-only solutions '
               TRACE_3 =  'Called in LIDORT_LPS_FOURIER, Fourier # '//CF
               STATUS = LIDORT_SERIOUS ; RETURN
             ENDIF
...
!   4/9/19. Note use of LP_TRANS_ATMOS_FINAL to go with adjusted water-leaving
!   4/9/19. Use PPSTREAMS post-processing mask
              ...
              CALL GET_LP_BOASOURCE &
                ( DO_USER_STREAMS, DO_SOLAR_SOURCES, DO_INCLUDE_THERMEMISS,       & ! Input flags sources
                  ...
                  NSTREAMS, NLAYERS, IBEAM, FOURIER, LAYER_TO_VARY, N_LAYER_WFS,  & ! Input
...
!  4/9/19. Use PPSTREAMS post-processing mask
             ...
             CALL UPUSER_PROFILEWF &
              ( DO_USER_STREAMS, DO_SOLAR_SOURCES, DO_MSMODE_LIDORT,                 & ! Input
                ...
                NSTREAMS, NLAYERS, N_USER_LEVELS, LAYER_TO_VARY, N_LAYER_WFS,        & ! Input
...
!  4/9/19. Use PPSTREAMS post-processing mask
             ...
             CALL GET_LP_TOASOURCE ( N_PPSTREAMS, PPSTREAM_MASK, IBEAM, N_LAYER_WFS, LP_TOA_SOURCE )
...
!  Exception handling
           ...
           IF ( STATUS_SUB .NE. LIDORT_SUCCESS ) THEN
              ...
              TRACE_2 = 'Error return from SURFACEWF_BVP_SOLUTION, '// &
                        'Called in LIDORT_LPS_FOURIER, Fourier # '//CF
              STATUS = LIDORT_SERIOUS ; RETURN
           ENDIF
...
!  4/9/19. Use PPSTREAMS post-processing mask
           ...
           CALL SURFACEWF_MASTER &
             ( DO_UPWELLING, DO_DNWELLING, DO_USER_STREAMS, DO_INCLUDE_MVOUTPUT,  & ! Input flags
               ...
               SURFACEWF_F, MEANI_DIFFUSE_SURFWF, FLUX_DIFFUSE_SURFWF)              ! Output
...
!  Linearized Green's function solution
            ...
            IF ( DO_PROFILE_LINEARIZATION ) THEN
              ...
              ( NSTREAMS, NMOMENTS, LAYER, FOURIER,               & ! Input
                ...
                DO_LAYER_SCATTERING, SOLARBEAM_CUTOFF,            & ! Input
...
!             write(*,*) 'Diff and Dirk',IBEAM,TRANSBEAM(IBEAM),TRANS
...
!     -- Get adjusted User-term surface-leaving contribution
            ...
            IF (  DO_INCLUDE_DIRECTSL .and. (DO_WATER_LEAVING .and..not.DO_EXTERNAL_WLEAVE) ) then
               ...
               ELSE
                  DO LUM = 1, N_PPSTREAMS
                     UM = PPSTREAM_MASK(LUM,IBEAM) ; SL_USERTERM(UM,IBEAM) =  USER_SLTERM_F_0(FOURIER,UM,IBEAM) * TFACTOR
                  ENDDO
               ENDIF
            ENDIF
...
!  4/9/19. Need to calculate LP_TRANS_ATMOS_FINAL for the water- leaving case
!          Assumes that (proportionally) LP derivatives are in the same ratio as those for the first-guess
!            This is an approximation for the iteration, but is exact for the Gordon result              
              ...
              IF ( FOURIER.eq.0 .and. DO_WATER_LEAVING ) THEN
                 ...
                 DO Q = 1, N_LAYER_WFS
                    LP_TRANS_ATMOS_FINAL(IBEAM,LAYER_TO_VARY,Q) = RATIO * LP_SOLARBEAM_BOATRANS(IBEAM,LAYER_TO_VARY,Q)
                 ENDDO
              ENDIF
...
!  Rob Fix, 01/06/14, Reorganized calling arguments, including TAYLOR_ORDER
              ...
              CALL LP_BVPTEL_SOLUTION_MASTER &
                ( DO_INCLUDE_SURFACE, DO_LOCALBEAM(IBEAM), DO_LAYER_SCATTERING,             & ! Input, Flags
                  DO_PLANE_PARALLEL, TAYLOR_ORDER, FOURIER, IBEAM, NLAYERS,                 & ! Input, Numbers
                  NSTREAMS, NSTREAMS_2, LAYER_TO_VARY, N_LAYER_WFS, N_SUPDIAG, N_SUBDIAG,   & ! Input, Numbers
...
!     -- Linearization similar to that in lp_Postprocessing (QUADINTENS_LEVEL_DN)
             ...
             IF ( DO_PLANETARY_PROBLEM ) THEN
               ...
               DO Q = 1, N_LAYER_WFS
                  ...
                  L_TRANS_DIRC = SUNLAYER_COSINES(N,IBEAM) * FLUX_FACTOR * LP_SOLARBEAM_ATRANS(IBEAM,K,Q)   ! Direct
                  LP_TRANSBEAM(IBEAM,K,Q) = L_TRANS_DIFF + L_TRANS_DIRC
!                  if ( k.eq.1.and.q.eq.1)write(*,*) 'Diff and Dirk',IBEAM,L_TRANS_DIFF,L_TRANS_DIRC
               ENDDO
             ENDIF
...
!  Surface WFs: Postprocessing Master. [DO_INCLUDE_DIRECTBEAM = .false. (automatic here)
!  4/9/19. Separate terms from surface (reflected-DB and Sleave)
!  4/9/19. Use PPSTREAMS post-processing mask
...
!   4/9/19. Routine completely rewritten
               ...
               CALL LIDORT_LSSL_WFS &
                ( DO_UPWELLING, DO_DNWELLING, DO_USER_STREAMS, DO_INCLUDE_MVOUTPUT,        & ! input flags
                  DO_WATER_LEAVING, DO_SL_ISOTROPIC, DO_BRDF_SURFACE, DO_INCLUDE_DIRECTSL, & ! input flags
                  ...
                  SURFACEWF_F, MEANI_DIFFUSE_SURFWF, FLUX_DIFFUSE_SURFWF, STATUS_SUB, MESSAGE, TRACE_1 ) ! outputs and exceptions
...
!  Exception handling
              ...
              IF ( STATUS_SUB .NE. LIDORT_SUCCESS ) THEN
                ...
                TRACE_2 = 'Error return from LIDORT_LSSL_WFS, '// &
                          'Beam Called in LIDORT_LPS_FOURIER, Fourier # '//CF
                STATUS = LIDORT_SERIOUS
                RETURN
              ENDIF
...
!  ######
      ...
      RETURN
END SUBROUTINE LIDORT_LPS_FOURIER

end MODULE LIDORT_LPS_masters_m
