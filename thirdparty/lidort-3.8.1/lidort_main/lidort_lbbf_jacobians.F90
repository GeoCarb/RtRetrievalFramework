      ...
      IF ( DO_INCLUDE_SURFACE.and.JB.ge.nlayers-1 ) THEN
        ...
        if ( jb .eq. nlayers ) then
         ...
         IF ( DO_BRDF_SURFACE  ) THEN
            ...
            do i = 1, nstreams
               FACTOR = SURFACE_FACTOR * Dot_Product(Down(1:nstreams),brdf_f(M,i,1:nstreams))
               Reflec(i) = FACTOR
            enddo
         ELSE
...
!  -------------------
      ...
      IF ( DO_BRDF_SURFACE ) THEN
         DO UM = 1, N_USER_STREAMS
            FACTOR = DOT_PRODUCT(L_IDOWN(1:nstreams),UBRDF_F(M,UM,1:NSTREAMS))
            L_BOA_MSSOURCE(UM) = SURFACE_FACTOR * FACTOR
         ENDDO
         IF ( DO_QTHTONLY ) THEN
            DO I = 1, NSTREAMS
              FACTOR = DOT_PRODUCT(L_IDOWN(1:nstreams),BRDF_F(M,I,1:NSTREAMS))
