...
!    --- Changes to the GET_LC_BOASOURCE to deal with WLeaving

module lidort_lc_wfatmos_m
...
!  Dependencies
   ...
   USE lidort_lc_PostProcessing_m
...
!  ---------------
      ...
      INTEGER    :: UTA, UM, Q, NC, UT, IB, LUM, M
      ...
      DO UTA = 1, N_USER_LEVELS
        DO Q = 1, N_TOTALCOLUMN_WFS
          DO LUM = 1, N_PPSTREAMS
            COLUMNWF_F(Q,UTA,LUM,IBEAM,UPIDX) = ZERO
          ENDDO
        ENDDO
      ENDDO
...
!  Layer source term
            ...
            ( DO_SOLAR_SOURCES, DO_INCLUDE_THERMEMISS, DO_THERMAL_TRANSONLY, & ! input
              ...
              IB, N, N_TOTALCOLUMN_WFS, NSTREAMS, N_PPSTREAMS, PPSTREAM_MASK,     & ! input, Numbers
...
!    DO_INCLUDE_MVOUTPUT = .TRUE. only for Fourier = 0 )
          ...
          IF ( DO_INCLUDE_MVOUTPUT ) THEN
           ...
           ( DO_SOLAR_SOURCES, DO_INCLUDE_THERMEMISS, DO_THERMAL_TRANSONLY,                 & ! Input
             TAYLOR_ORDER, NSTREAMS, NLAYERS, IB, UTA, UT, N, N_TOTALCOLUMN_WFS,                 & ! Input
...
!  Cumulative and final
            ...
            DO LUM = 1, N_PPSTREAMS
               ...
               DO Q = 1, N_TOTALCOLUMN_WFS
                  ...
                  COLUMNWF_F(Q,UTA,LUM,IB,UPIDX) = FLUX_MULTIPLIER * L_FINAL_SOURCE
               ENDDO
            ENDDO
...
!    DO_INCLUDE_MVOUTPUT = .TRUE. only for Fourier = 0 )
          ...
          IF ( DO_INCLUDE_MVOUTPUT ) THEN
            ...
            ( DO_THERMAL_TRANSONLY,  FLUX_MULTIPLIER,                & ! input
              NLAYERS, NSTREAMS, IB, UTA, NLEVEL, N_TOTALCOLUMN_WFS,      & ! input
...
!  User-defined stream output, just set to the cumulative source term
          ...
          IF ( DO_USER_STREAMS ) THEN
             DO LUM = 1, N_PPSTREAMS
                UM = PPSTREAM_MASK(LUM,IB)
                DO Q = 1, N_TOTALCOLUMN_WFS
                   COLUMNWF_F(Q,UTA,LUM,IB,UPIDX) = FLUX_MULTIPLIER * L_CUMUL_SOURCE(UM,Q)
                ENDDO
             ENDDO
          ENDIF

!  End ongrid/offgrid clause
...
!  Zero all Fourier component output
      ...
      DO UTA = 1, N_USER_LEVELS
        DO Q = 1, N_TOTALCOLUMN_WFS
          DO LUM = 1, N_PPSTREAMS
            COLUMNWF_F(Q,UTA,LUM,IB,DNIDX) = ZERO
          ENDDO
        ENDDO
      ENDDO
...
!    DO_INCLUDE_MVOUTPUT = .TRUE. only for Fourier = 0 )
          ...
          IF ( DO_INCLUDE_MVOUTPUT ) THEN
            ...
            ( DO_SOLAR_SOURCES, DO_INCLUDE_THERMEMISS, DO_THERMAL_TRANSONLY,                 & ! Input
              TAYLOR_ORDER, NSTREAMS, IB, UTA, UT, N, N_TOTALCOLUMN_WFS,                          & ! Input
              QUAD_STREAMS, FLUX_MULTIPLIER, PARTAU_VERT, L_DELTAU_VERT, SOLARBEAM_CUTOFF,   & ! Input
            ...
            DO LUM = 1, N_PPSTREAMS
               UM = PPSTREAM_MASK(LUM,IBEAM)
                DO Q = 1, N_TOTALCOLUMN_WFS
                   ...
                   COLUMNWF_F(Q,UTA,LUM,IB,DNIDX) = FLUX_MULTIPLIER * L_FINAL_SOURCE
               ENDDO
            ENDDO
...
!    DO_INCLUDE_MVOUTPUT = .TRUE. only for Fourier = 0 )
          ...
          IF ( DO_INCLUDE_MVOUTPUT ) THEN
            ...
            ( DO_THERMAL_TRANSONLY, FLUX_MULTIPLIER,      & ! Input
              NSTREAMS, IB, UTA, NLEVEL, N_TOTALCOLUMN_WFS,    & ! Input
...
!  User-defined stream output, just set to the cumulative source term
          ...
          IF ( DO_USER_STREAMS ) THEN
             DO LUM = 1, N_PPSTREAMS
                UM = PPSTREAM_MASK(LUM,IBEAM)
                DO Q = 1, N_TOTALCOLUMN_WFS
                   COLUMNWF_F(Q,UTA,LUM,IB,DNIDX) = FLUX_MULTIPLIER * L_CUMUL_SOURCE(UM,Q)
                ENDDO
             ENDDO
          ENDIF
...
SUBROUTINE MIFLUX_COLUMNWF                                           &
           ( DO_UPWELLING, DO_DNWELLING, DO_INCLUDE_DIRECTBEAM,      & ! Input
             IB, N_TOTALCOLUMN_WFS, NSTREAMS, N_USER_LEVELS, FLUX_FACTOR, & ! Input
      ...
      IF ( DO_UPWELLING ) THEN
        DO UTA = 1, N_USER_LEVELS
         DO Q = 1, N_TOTALCOLUMN_WFS
          ...
          ENDDO
          MEANI_DIFFUSE_COLWF(Q,UTA,IB,UPIDX) = SM * HALF
          FLUX_DIFFUSE_COLWF (Q,UTA,IB,UPIDX) = SF * PI2
         ENDDO
        ENDDO
      ENDIF
...
!  Diffuse term contribution
        ...
        DO UTA = 1, N_USER_LEVELS
         DO Q = 1, N_TOTALCOLUMN_WFS
          ...
          ENDDO
          MEANI_DIFFUSE_COLWF(Q,UTA,IB,DNIDX) = SM * HALF
          FLUX_DIFFUSE_COLWF (Q,UTA,IB,DNIDX) = SF * PI2
         ENDDO
        ENDDO
...
!    LC_INITIAL_TRANS is a logarithmic derivative
...
!     LTRANS, Placeholder filled, 1/9/18
                ...
                HELP = LC_T_UTDN_MUBAR(UT,IB,Q) + LC_INITIAL_TRANS(N,IB,Q) * T_UTDN_MUBAR(UT,IB)
                L_TRANS_SCALED = HELP * INITIAL_TRANS(N,IB)
                L_TRANS        = LC_PARTIALS_SOLARTRANS(UT,IB,Q) ! Filled PLACEHOLDER, 1/9/18
...
!  direct calculation with non-scaled linearized transmittances
                ...
                DNMEANI_DIRECT_COLWF(Q,UTA,IB) = L_DIRECT_MEANI
                DNFLUX_DIRECT_COLWF( Q,UTA,IB) = L_DIRECT_FLUX

!  Diffuse calculation

                L_DIRECT_MEANI_SCALED = FLUX_FACTOR * L_TRANS_SCALED / PI4
                L_DIRECT_FLUX_SCALED  = FMU0  * L_TRANS_SCALED
                MEANI_DIFFUSE_COLWF(Q,UTA,IB,DNIDX) = &
                     MEANI_DIFFUSE_COLWF(Q,UTA,IB,DNIDX) + ( L_DIRECT_MEANI_SCALED - L_DIRECT_MEANI )
                FLUX_DIFFUSE_COLWF (Q,UTA,IB,DNIDX) = &
                     FLUX_DIFFUSE_COLWF (Q,UTA,IB,DNIDX) + ( L_DIRECT_FLUX_SCALED  - L_DIRECT_FLUX  )
...
!  direct transmittances, scaled and unscaled.
                  ...
                  HELP = LC_T_DELT_MUBAR(N,IB,Q) + LC_INITIAL_TRANS(N,IB,Q) * T_DELT_MUBAR(N,IB)
                  L_TRANS_SCALED = HELP * INITIAL_TRANS(N,IB)
                  L_TRANS        = LC_LEVELS_SOLARTRANS(N,IB,Q)
...
SUBROUTINE GET_LC_TOASOURCE &
     ( N_PPSTREAMS, PPSTREAM_MASK, IBEAM, N_TOTALCOLUMN_WFS, LC_TOA_SOURCE )
...
!  Linearized transmittance flux for water-leaving
      ...
      REAL(fpk), intent(in)  :: LC_TRANS_ATMOS_FINAL (MAXBEAMS,MAX_ATMOSWFS)
...
!  ---------------
      ...
      LOGICAL   :: DO_QTHTONLY
      INTEGER   :: M, N, J, I, UM, AA, Q, IB, K1, LAY, LUM
...
!  initialise linearized BOA source functions
      ...
      IF ( DO_USER_STREAMS ) THEN
         DO LUM = 1, N_PPSTREAMS
            UM = PPSTREAM_MASK(LUM,IBEAM)
            L_BOA_MSSOURCE(UM,1:N_TOTALCOLUMN_WFS) = ZERO
            L_BOA_DBSOURCE(UM,1:N_TOTALCOLUMN_WFS) = ZERO
         ENDDO
      ENDIF
...
!  Thermal tranmsittance only, special term
      ...
      IF ( DO_QTHTONLY ) THEN
         DO I = 1, NSTREAMS
            L_BOA_THTONLY_SOURCE(I,1:N_TOTALCOLUMN_WFS) = ZERO
         ENDDO
      ENDIF
...
!  Add reflected direct beam if flagged
      ...
      IF ( DO_INCLUDE_DIRECTRF .AND. DO_USER_STREAMS ) THEN
         DO LUM = 1, N_PPSTREAMS
            UM = PPSTREAM_MASK(LUM,IB)
            FAC = - RF_USER_DIRECT_BEAM(UM,IB)
            DO Q = 1, N_TOTALCOLUMN_WFS
               L_BEAM = DOT_PRODUCT(L_DELTAU_VERT(Q,1:N),DELTAU_SLANT(N,1:N,IB))
               L_BOA_DBSOURCE(UM,Q) = L_BOA_DBSOURCE(UM,Q) + L_BEAM * FAC
            ENDDO
         ENDDO
      ENDIF
...
!  Add surface-term linearization if flagged
      ...
      IF ( DO_INCLUDE_DIRECTSL .AND. DO_USER_STREAMS ) THEN
         DO LUM = 1, N_PPSTREAMS
            ...
            DO Q = 1, N_TOTALCOLUMN_WFS
               L_BOA_DBSOURCE(UM,Q) = L_BOA_DBSOURCE(UM,Q) + LC_TRANS_ATMOS_FINAL(IB,Q) * FAC
            ENDDO
         ENDDO
      ENDIF
              ...
              DO UT = 1, N_USER_LEVELS
                DO I = 1, N_USER_STREAMS
                  DO UA = 1, LOCAL_N_USERAZM
                    V = UMOFF(IBEAM,I) + UA
                    COLUMNWF(Q,UT,V,W) = COLUMNWF_F(Q,UT,I,IBEAM,W)
                  ENDDO
                ENDDO
              ENDDO
            ENDDO
            ...
            DO IDIR = 1, N_DIRECTIONS
              ...
              DO UT = 1, N_USER_LEVELS
                DO I = 1, N_USER_STREAMS
                  DO UA = 1, LOCAL_N_USERAZM
                    V = UMOFF(IBEAM,I) + UA
                    !CALL TP15E (FOURIER,Q,UT,V,W,COLUMNWF,COLUMNWF_SS)
                    COLUMNWF(Q,UT,V,W) = COLUMNWF(Q,UT,V,W) + COLUMNWF_SS(Q,UT,V,W)
                  ENDDO
                ENDDO
              ENDDO
            ENDDO
          ...
          DO Q = 1, N_TOTALCOLUMN_WFS
            DO UT = 1, N_USER_LEVELS
              DO I = 1, N_USER_STREAMS
                DO UA = 1, LOCAL_N_USERAZM
                  V = UMOFF(IBEAM,I) + UA
                  !CALL TP15F (FOURIER,Q,UT,V,COLUMNWF,COLUMNWF_DB)
                  COLUMNWF(Q,UT,V,UPIDX) = COLUMNWF(Q,UT,V,UPIDX) + COLUMNWF_DB(Q,UT,V)
                ENDDO
              ENDDO
            ENDDO
          ENDDO
          ...
          DO UA = 1, LOCAL_N_USERAZM
            DO IDIR = 1, N_DIRECTIONS
             ...
             DO UT = 1, N_USER_LEVELS
              DO I = 1, N_USER_STREAMS
               V = UMOFF(IBEAM,I) + UA
               !CALL TP15G (FOURIER,Q,UT,I,IBEAM,V,W,COLUMNWF,COLUMNWF_F)
...
SUBROUTINE LIDORT_LC_CONVERGE_OBSGEO &
      ( DO_FOCORR, DO_FOCORR_ALONE,                        & ! Input flags
        DO_UPWELLING, DO_NO_AZIMUTH,                       & ! Input flags
        ...
        IF ( .NOT. DO_FOCORR_ALONE ) THEN
          DO Q = 1, N_TOTALCOLUMN_WFS
            DO IDIR = 1, N_DIRECTIONS
              W = WHICH_DIRECTIONS(IDIR)
              DO UT = 1, N_USER_LEVELS
                COLUMNWF(Q,UT,IBEAM,W) = COLUMNWF_F(Q,UT,LUM,IBEAM,W)
              ENDDO
            ENDDO
          ENDDO
        ELSE
        ...
        IF ( DO_FOCORR ) THEN
          DO Q = 1, N_TOTALCOLUMN_WFS
            DO IDIR = 1, N_DIRECTIONS
              W = WHICH_DIRECTIONS(IDIR)
              DO UT = 1, N_USER_LEVELS
                !CALL TP15E2 (FOURIER,Q,UT,IBEAM,W,COLUMNWF,COLUMNWF_SS)
                COLUMNWF(Q,UT,IBEAM,W) = COLUMNWF(Q,UT,IBEAM,W) + COLUMNWF_SS(Q,UT,IBEAM,W)
              ENDDO
            ENDDO
          ENDDO
        ENDIF
...
!        IF ( DO_UPWELLING ) THEN
        ...
        IF ( DO_FOCORR .AND.DO_UPWELLING ) THEN
          DO Q = 1, N_TOTALCOLUMN_WFS
            DO UT = 1, N_USER_LEVELS
              !CALL TP15F2 (FOURIER,Q,UT,IBEAM,COLUMNWF,COLUMNWF_DB)
              COLUMNWF(Q,UT,IBEAM,UPIDX) = COLUMNWF(Q,UT,IBEAM,UPIDX) + COLUMNWF_DB(Q,UT,IBEAM)
            ENDDO
          ENDDO
        ENDIF
...
!  Add next Fourier component to output
        ...
        DO Q = 1, N_TOTALCOLUMN_WFS
          DO IDIR = 1, N_DIRECTIONS
            W = WHICH_DIRECTIONS(IDIR)
            DO UT = 1, N_USER_LEVELS
              !CALL TP15G2 (FOURIER,Q,UT,LUM,IBEAM,W,COLUMNWF,COLUMNWF_F)
              COLUMNWF(Q,UT,IBEAM,W) = COLUMNWF(Q,UT,IBEAM,W) + &
                COLUMNWF_F(Q,UT,LUM,IBEAM,W)*AZMFAC(LUM,IBEAM,LUA)
            ENDDO
          ENDDO
        ENDDO
...
END SUBROUTINE LIDORT_LC_CONVERGE_OBSGEO

!  End

end module lidort_lc_wfatmos_m
