...
!  Version 3.8: LIDORT module dependencies
   ...
   USE lidort_lc_wfatmos_m  , only : LIDORT_LC_CONVERGE, LIDORT_LC_CONVERGE_OBSGEO
...
!   4/30/19 Add SOLARBEAM_ATRANS for LC_SOLARBEAM_ATRANS output
      ...
      IF ( DO_COLUMN_LINEARIZATION ) THEN
        ...
        ( DO_PLANE_PARALLEL, NLAYERS, NBEAMS, N_PARTLAYERS,        & ! Input
          ...
          AVERAGE_SECANT, SOLARBEAM_CUTOFF,                        & ! Input
...
!              Added input, water-leaving control
            ...
            CALL SFO_LCS_MASTER_INTERFACE ( &
                ...
                FO_CUMTRANS, FO_SLTERM, FO_LC_CUMTRANS, FO_LSSL_SLTERM,               & ! Output Auxiliary 4/9/19 New line
...
!    -- Associated outputs are TRANSBEAM, ALBMED_USER, ALBMED_FLUXES, TRNMED_USER, TRNMED_FLUXES   
           ...
           CALL LIDORT_LCS_FOURIER ( FOURIER, &
            ...
            DO_COLUMN_LINEARIZATION, DO_SURFACE_LINEARIZATION, DO_SLEAVE_WFS,                       & !Input flags Linearization
            ...
            N_TOTALCOLUMN_WFS, LAYER_VARY_FLAG, LAYER_VARY_NUMBER, N_SURFACE_WFS, N_SLEAVE_WFS,     & !Input Linearization
            ...
            INTENSITY_F, MEANI_DIFFUSE, FLUX_DIFFUSE, DNMEANI_DIRECT, DNFLUX_DIRECT,                     & ! Output Ints/Fluxes
            COLUMNWF_F, MEANI_DIFFUSE_COLWF, FLUX_DIFFUSE_COLWF,DNMEANI_DIRECT_COLWF,DNFLUX_DIRECT_COLWF,& ! Output LC Jacobians
            SURFACEWF_F, MEANI_DIFFUSE_SURFWF, FLUX_DIFFUSE_SURFWF,                                      & ! Output LS Jacobians
            ...
            LC_TRANS_ATMOS_FINAL, LC_TRANSBEAM, LC_ALBMED_USER, LC_ALBMED_FLUXES, LC_TRNMED_USER, LC_TRNMED_FLUXES, & !Output 4/26/19
                   ...
                   if ( do_column_linearization ) then
                      ...
                      do uta = 1, n_user_levels
                         DO Q = 1, NWFS
                            ...
                            COLUMNWF_DB(Q,UTA,IB) = COLUMNWF_DB(Q,UTA,IB) + L_CUMSOURCE_DB
                         enddo
                      enddo
                   endif
                      ...
                      do uta = 1, n_user_levels
                         do g = g1, g2
                            DO Q = 1, NWFS
                               ...
                               COLUMNWF_DB(Q,UTA,G) = COLUMNWF_DB(Q,UTA,G) + L_CUMSOURCE_DB
                            enddo
                         enddo
                      enddo
                   endif
          ...
          IF ( FOURIER .GT. 0 ) THEN
            ...
            IF ( .not. DO_OBSERVATION_GEOMETRY ) THEN
              DO UA = 1, LOCAL_N_USERAZM
                AZMFAC(1:N_USER_STREAMS,1:NBEAMS,UA) = COS(DEG_TO_RAD*DFC*USER_RELAZMS(UA))
              ENDDO
            ELSE
              DO IB = 1, NBEAMS
...
!  Fourier summation of Column jacobians

                IF ( DO_COLUMN_LINEARIZATION ) THEN
                  CALL LIDORT_LC_CONVERGE &
                  ( DO_FOCORR, DO_FOCORR_ALONE, DO_UPWELLING, DO_NO_AZIMUTH, & ! Input flags
                    N_USER_STREAMS, N_USER_LEVELS, LOCAL_N_USERAZM,          & ! Input numbers
                    IBEAM, FOURIER, N_TOTALCOLUMN_WFS,                       & ! Input numbers
                    VZA_OFFSETS, N_DIRECTIONS, WHICH_DIRECTIONS,             & ! Input bookkeeping
                    AZMFAC, COLUMNWF_F, COLUMNWF_SS, COLUMNWF_DB,            & ! Input Azm, fields
                    COLUMNWF )                                                 ! Output
                ENDIF
...
!  Fourier summation of Column jacobians
                ...
                IF ( DO_COLUMN_LINEARIZATION ) THEN
                  CALL LIDORT_LC_CONVERGE_OBSGEO &
                   ( DO_FOCORR, DO_FOCORR_ALONE, DO_UPWELLING, DO_NO_AZIMUTH, & ! Input flags
                     N_USER_LEVELS, IBEAM, FOURIER,                           & ! Input numbers
                     N_TOTALCOLUMN_WFS, N_DIRECTIONS, WHICH_DIRECTIONS,       & ! Input bookkeeping
                     AZMFAC, COLUMNWF_F, COLUMNWF_SS, COLUMNWF_DB,            & ! Input Azm, fields
                     COLUMNWF )                                                ! Output
                ENDIF
...
SUBROUTINE LIDORT_LCS_FOURIER ( FOURIER, &
            ...
            L_T_DELT_DISORDS, L_T_DISORDS_UTUP, L_T_DISORDS_UTDN,                                        & ! Input L Trans.
            LC_EMULT_UP, LC_EMULT_DN, LC_UT_EMULT_UP, LC_UT_EMULT_DN,                                    & ! Input LC Trans.
            INTENSITY_F, MEANI_DIFFUSE, FLUX_DIFFUSE, DNMEANI_DIRECT, DNFLUX_DIRECT,                     & ! Output 
            COLUMNWF_F, MEANI_DIFFUSE_COLWF, FLUX_DIFFUSE_COLWF,DNMEANI_DIRECT_COLWF,DNFLUX_DIRECT_COLWF,& ! Output LC
            SURFACEWF_F, MEANI_DIFFUSE_SURFWF, FLUX_DIFFUSE_SURFWF,                                      & ! Output LS
            ABBWFS_JACOBIANS, ABBWFS_FLUXES, SBBWFS_JACOBIANS, SBBWFS_FLUXES,                            & ! Output LBBF Jacobians
            TRANS_ATMOS_FINAL, TRANSBEAM, ALBMED_USER, ALBMED_FLUXES, TRNMED_USER, TRNMED_FLUXES,                   & !Output 4/26/19
            LC_TRANS_ATMOS_FINAL, LC_TRANSBEAM, LC_ALBMED_USER, LC_ALBMED_FLUXES, LC_TRNMED_USER, LC_TRNMED_FLUXES, & !Output 4/26/19
...
!  Linearized Green's function multipliers for off-grid optical depths
      ...
      REAL(fpk)  :: LC_UT_GMULT_UP (MAXSTREAMS,MAX_PARTLAYERS,MAX_ATMOSWFS)
      REAL(fpk)  :: LC_UT_GMULT_DN (MAXSTREAMS,MAX_PARTLAYERS,MAX_ATMOSWFS)
...
!  .. error tracing
            ...
            IF ( STATUS_SUB .NE. LIDORT_SUCCESS ) THEN
               write(CF,'(I2)')FOURIER
               TRACE_2 =  'LIDORT_HOM_SOLUTION Called in LIDORT_LCS_FOURIER, Fourier component '//CF
...
!  .. error tracing
               ...
               IF ( STATUS_SUB .NE. LIDORT_SUCCESS ) THEN
                  write(CF,'(I2)')FOURIER
                  TRACE_2 =  'LIDORT_L_HOM_SOLUTION Called in LIDORT_LCS_FOURIER, Fourier component '//CF
                  TRACE_3 =  'LIDORT_LCS_FOURIER call '
                  STATUS  = LIDORT_SERIOUS ; RETURN
               ENDIF
...
!  4/9/19. Use PPSTREAMS post-processing mask
            ...
            CALL UPUSER_COLUMNWF &
              ( DO_USER_STREAMS, DO_SOLAR_SOURCES, DO_MSMODE_LIDORT,              & ! Input
                ...
                NSTREAMS, NLAYERS, N_USER_LEVELS, N_TOTALCOLUMN_WFS, N_PPSTREAMS, & ! Input
                PPSTREAM_MASK,  DO_LAYER_SCATTERING, UTAU_LEVEL_MASK_UP,          & ! Input
                PARTLAYERS_OUTFLAG, PARTLAYERS_OUTINDEX, PARTLAYERS_LAYERIDX,     & ! Input
                ...
                L_U_XPOS, L_U_XNEG, LC_U_WPOS1, NCON, PCON,                & ! Input
                ...
                L_BOA_THTONLY_SOURCE, LC_BOA_MSSOURCE, LC_BOA_DBSOURCE,        & ! Input
...
!  4/9/19. Use PPSTREAMS post-processing mask
            ...
            CALL DNUSER_COLUMNWF &
              ( DO_USER_STREAMS, DO_SOLAR_SOURCES, DO_MSMODE_LIDORT,              & ! Input
                DO_INCLUDE_THERMEMISS, DO_THERMAL_TRANSONLY, DO_INCLUDE_MVOUTPUT, & ! Input
                NSTREAMS, NLAYERS, N_USER_LEVELS, N_TOTALCOLUMN_WFS, N_PPSTREAMS, & ! Input
                PPSTREAM_MASK, DO_LAYER_SCATTERING, UTAU_LEVEL_MASK_DN,           & ! Input
                PARTLAYERS_OUTFLAG, PARTLAYERS_OUTINDEX, PARTLAYERS_LAYERIDX,     & ! Input
                FOURIER, IBEAM, TAYLOR_ORDER, FLUX_MULTIPLIER, QUAD_STREAMS,      & ! Input
                PARTAU_VERT, DELTAU_VERT, L_DELTAU_VERT,                          & ! Input
                USER_SECANTS, SOLARBEAM_CUTOFF, INITIAL_TRANS, AVERAGE_SECANT,    & ! Input
                ...
                L_KEIGEN, L_ATERM_SAVE, L_BTERM_SAVE,               & ! Input
                L_U_XPOS, L_U_XNEG, LC_U_WNEG1, NCON, PCON,                & ! Input
                ...
                L_T_WLOWER, LC_TOA_SOURCE,                                 & ! Input
                FLAGS_LC_GMULT, LC_UT_GMULT_UP, LC_UT_GMULT_DN,            & ! Output
                COLUMNWF_F, QUADCOLUMNWF )  
...
!  4/9/19. Use PPSTREAMS post-processing mask
              ...
              ( DO_USER_STREAMS, DO_SOLAR_SOURCES, DO_MSMODE_LIDORT,              & ! Input
                ...
                L_HMULT_1, L_HMULT_2, LC_EMULT_UP,                         & ! Input
                L_UT_HMULT_UU, L_UT_HMULT_UD, LC_UT_EMULT_UP,              & ! Input
                L_LAYER_TSUP_UP, L_LAYER_TSUP_UTUP, L_UT_T_PARTIC, L_T_WUPPER, & ! Input                   & ! Input
                L_BOA_THTONLY_SOURCE, LC_BOA_MSSOURCE, LC_BOA_DBSOURCE,        & ! Input
                FLAGS_LC_GMULT, LC_UT_GMULT_UP, LC_UT_GMULT_DN,            & ! Output
                COLUMNWF_F, QUADCOLUMNWF )                                   ! Output
...
!   4/9/19. Routine completely rewritten for Version 3.8.1. See inside for details.
              ...
              CALL LIDORT_LSSL_WFS &
                ( DO_UPWELLING, DO_DNWELLING, DO_USER_STREAMS, DO_INCLUDE_MVOUTPUT,        & ! input flags
                  ...
                  FLUX_MULTIPLIER, UTAU_LEVEL_MASK_UP, UTAU_LEVEL_MASK_DN,                 & ! Inputs bookkeeping
