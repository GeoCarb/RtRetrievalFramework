...
!    --- New routine BVP_ADJDUSTED_BACKSUB, for LWCoupling
      ...
      IF ( DO_BRDF_SURFACE ) THEN
        DO AA = 1, NSTREAMS
          DO I = 1, NSTREAMS
            ...
            R2_HOMP(I,AA) = SURFACE_FACTOR * DOT_PRODUCT ( H_XPOS(1:NSTREAMS,AA), BRDF_F(FOURIER,I,1:NSTREAMS) )
            R2_HOMM(I,AA) = SURFACE_FACTOR * DOT_PRODUCT ( H_XNEG(1:NSTREAMS,AA), BRDF_F(FOURIER,I,1:NSTREAMS) )
          ENDDO
        ENDDO
      ENDIF
...
!mick eff 3/22/2017 - loops in both IF and ELSE sections
       IF ( DO_INCLUDE_SURFACE ) THEN
         ...
         !    SMAT2(CP,CEP) = XPNET * T_DELT_EIGEN(EP,N)
         !    SMAT2(CP,CEM) = XMNET
         !  ENDDO
         !ENDDO
         DO EP = 1, NSTREAMS
...
!  module, dimensions and numbers
      ...
      USE LIDORT_pars_m, only : MAXSTREAMS, MAXSTREAMS_2, MAXMOMENTS,        &
                                ...
                                LIDORT_SUCCESS, LIDORT_SERIOUS, ZERO, ONE
...
!  module, dimensions and numbers
      ...
      USE LIDORT_pars_m, only : MAXSTREAMS, MAXSTREAMS_2, MAXMOMENTS, MAXLAYERS, ZERO
...
!  For bidirectional reflecting surface
      ...
      IF ( DO_BRDF_SURFACE ) THEN
        DO I = 1, NSTREAMS
          R2_PARTIC(I) = DOT_PRODUCT(H_WLOWER(1:NSTREAMS),BRDF_F(FOURIER,I,1:NSTREAMS)) * SURFACE_FACTOR
        ENDDO
      ENDIF
...
!  Special Fourier = 0 component calculation for the adjusted atmospheric downwelling Flux at BOA
      ...
      USE LIDORT_PARS_m, Only : MAXMOMENTS, MAXLAYERS, MAXSTREAMS, MAXBEAMS, &
...
!  Helper variables
      ...
      INTEGER        :: JITER, I, I1, K, C0, C1, CM, INFO, STATUS_SUB
...
!  Find adjusted surface-leaving contribution
         ...
         ELSE
            DO I = 1, NSTREAMS
               SL_QUADTERM(I,IBEAM) = SLTERM_F_0(FOURIER,I,IBEAM) * TFACTOR
            ENDDO
         ENDIF
...
!  module, dimensions and numbers
      ...
      USE LIDORT_pars_m, only : MAXSTREAMS, MAXSTREAMS_2, MAXMOMENTS,  &
                                MAXLAYERS, MAXBANDTOTAL, MAXTOTAL, ZERO, ONE,  &
                                LIDORT_SUCCESS, LIDORT_SERIOUS
...
!  Surface condition
      ...
      IF ( DO_INCLUDE_SURFACE ) THEN
         DO AA = 1, NSTREAMS
            DO I = 1, NSTREAMS
               R2_HOMP(I,AA) = CUMTRANS(I) * SURFACE_FACTOR * DOT_PRODUCT(H_XPOS(1:NSTREAMS,AA),BRDF_F(M,I,1:NSTREAMS))
               R2_HOMM(I,AA) = CUMTRANS(I) * SURFACE_FACTOR * DOT_PRODUCT(H_XNEG(1:NSTREAMS,AA),BRDF_F(M,I,1:NSTREAMS))
            ENDDO
         ENDDO
      ENDIF
...
!   R2_HOMM and R2_HOMP added to list, 5/24/16, Version 3.8
      ...
      CALL BVPTEL_MATRIX_SETUP                                 &
          ( DO_INCLUDE_SURFACE, NSTREAMS, NSTREAMS_2, NLAYERS, & ! Input
...
!  module, dimensions and numbers
      ...
      USE LIDORT_pars_m, only : MAXSTREAMS, MAXSTREAMS_2, MAXBEAMS, MAXMOMENTS, &
                                MAXLAYERS, MAXBANDTOTAL, MAXTOTAL, ZERO, ONE,   &
                                LIDORT_SUCCESS, LIDORT_SERIOUS
...
!    --  Add cumulative transmittance to surface (through non-scattering layers)
      ...
      IF ( DO_INCLUDE_SURFACE ) THEN
         ...
         DO I = 1, NSTREAMS
            R2_PARTIC(I) = CUMTRANS(I) * SURFACE_FACTOR * DOT_PRODUCT(H_WLOWER(1:NSTREAMS),BRDF_F(FOURIER,I,1:NSTREAMS))
         ENDDO
      ENDIF
