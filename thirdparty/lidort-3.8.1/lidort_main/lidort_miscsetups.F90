...
SUBROUTINE LIDORT_DIRECTRADIANCE ( DO_USER_STREAMS, &
            ...
            SOLARBEAM_ATRANS, DO_REFLECTED_DIRECTBEAM,      & ! input
...
!  4/9/19. Not done here for Adjustable water-leaving contributions (done later)
         ...
         IF ( DO_CALC_SLTERMS ) THEN
            ...
            ELSE
               DO I = 1, NSTREAMS
                  SL_QUADTERM(I,IB) = SLTERM_F_0(M,I,IB) * HELP
               ENDDO
               IF ( DO_USER_STREAMS ) THEN
                  DO LUI = 1, N_PPSTREAMS
                     UI = PPSTREAM_MASK(LUI,IB) ; SL_USERTERM(UI,IB) = USER_SLTERM_F_0(M,UI,IB) *  HELP
                  ENDDO
               ENDIF
            ENDIF
         ENDIF
