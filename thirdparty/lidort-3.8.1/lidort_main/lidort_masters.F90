...
!  Local integers
      ...
      INTEGER         :: OFF, UA, UM, IB, G, G1, G2, UTA, LUM, LUA, NMOMS, NMOMSINP
...
!              Added input, water-leaving control
            ...
            CALL SFO_MASTER_INTERFACE ( &
                ...
                DO_SL_ISOTROPIC, NLAYERS, NFINELAYERS, NMOMENTS_INPUT, NBEAMS, BEAM_SZAS,           & ! Input numbers
                ...
                FO_INTENSITY_ATMOS, FO_INTENSITY_SURF, FO_INTENSITY, FO_CUMTRANS, FO_SLTERM,        & ! Output Intensity
...
!mick note 3/22/2017 - Important!  FO_INTENSITY_SURF now contains BOTH solar AND thermal
...
!    -- Associated outputs are TRANSBEAM, ALBMED_USER, ALBMED_FLUXES, TRNMED_USER, TRNMED_FLUXES   
          ...
          CALL LIDORT_FOURIER ( FOURIER, &
            ...
            NSTREAMS_2, NTOTAL, N_SUBDIAG, N_SUPDIAG, N_PARTLAYERS, N_PPSTREAMS, PPSTREAM_MASK,     & !Input Bookkeeping
            TAYLOR_ORDER, FLUX_FACTOR, BEAM_COSINES, SUNLAYER_COSINES, LOCAL_CSZA,                  & !Input (SZA and flux)
            USER_STREAMS, USER_SECANTS, QUAD_STREAMS, QUAD_WEIGHTS, QUAD_STRMWTS,                   & !Input (Streams)
            ...
            SURFACE_BB_INPUT, EMISSIVITY, USER_EMISSIVITY,                                          & !Input (Surface thermal)
            DELTAU_VERT, PARTAU_VERT, DELTAU_POWER, XTAU_POWER, OMEGA_MOMS,                         & !Input (Atmos optical)
            ...
            SOLARBEAM_BOATRANS, SOLARBEAM_ATRANS, LEVELS_SOLARTRANS, PARTIALS_SOLARTRANS,              & !Input (Beam Stuff)
            ...
            INTENSITY_F, MEANI_DIFFUSE, FLUX_DIFFUSE, DNMEANI_DIRECT, DNFLUX_DIRECT,                & !Output MAIN
...
!  useful debug
!        if ( FOURIER.eq.0)write(*,*)FOURIER,TRANS_ATMOS_FINAL(1),INTENSITY_F(1:2,1,1,1),INTENSITY_F(2,1,1,2)
...
!         LIDORT_Out%Main%TS_SPHERALB                      = SPHERALB
!         LIDORT_Out%Main%TS_TRANS1_BEAM(1:NBEAMS)         = TRANS1_BEAM(1:NBEAMS)
!         LIDORT_Out%Main%TS_TRANS1_USER(1:N_USER_STREAMS) = TRANS1_USER(1:N_USER_STREAMS)
!      ENDIF
...
!  .. error tracing
        ...
        IF ( STATUS_SUB .NE. LIDORT_SUCCESS ) THEN
          write(CF,'(I2)')FOURIER
          TRACE_2 =  'LIDORT_HOM_SOLUTION Called in LIDORT_FOURIER, Fourier component '//CF
...
!             if ( FOURIER.eq.0.and.IBEAM.eq.1)write(*,*)'BOA',IDOWNSURF(1), RF_USER_DIRECT_BEAM(IBEAM,1)
