...
!    - Use of adjusted water-leaving trans-atmos dependency.
      ...
      SUBROUTINE LIDORT_LSSL_WFS &
        ( DO_UPWELLING, DO_DNWELLING, DO_USER_STREAMS, DO_INCLUDE_MVOUTPUT,        & ! input flags
          ...
          SURFACEWF_F, MINT_SURFACEWF, FLUX_SURFACEWF, STATUS, MESSAGE, TRACE )      ! outputs and exceptions
      ...
      ELSE
         ...
         IF ( DO_USER_STREAMS .and. DO_UPWELLING .and. DO_INCLUDE_DIRECTSL ) THEN
            DO LUM = 1, N_PPSTREAMS
               ...
               DO Q = 1, N_SLEAVE_WFS
                  SL = LSSL_SLTERM_ISOTROPIC(Q,IB) * HELP
                  LSSL_USERTERM(Q,UM) = LSSL_USER_SLTERM_F_0(Q,M,UM,IB) * HELP
               ENDDO
            ENDDO
         ENDIF   
      ENDIF
...
!     Lambertian case, same for all user-streams
         ...
         IF ( DO_BRDF_SURFACE ) THEN
            DO Q = 1, N_SLEAVE_WFS
               DO LUM = 1, N_PPSTREAMS
                  UM = PPSTREAM_MASK(LUM,IB)
                  REFLEC = SURFACE_FACTOR * DOT_PRODUCT(INTEGRAND(Q,1:NSTREAMS), USER_BRDF_F(M,UM,1:NSTREAMS))
