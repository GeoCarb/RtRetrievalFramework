...
! ############################################################### Old
! # Subroutines in the (former) LP_CORRECTION Module            # Old
! #                                                             # Old
! #       Version 3.4.  LP module  (profile Jacobians)          # Old
! #             LIDORT_LP_SSCORR_NADIR (master, new 2.4)        # Old
! #                                                             # Old
! #      Version 3.2   Whole layer integration                  # Old
! #              3.3.  partial-layer integration                # Old
! #      Version 3.4.  Column Jacobians introduced              # Old
! #             LIDORT_LP_SSCORR_OUTGOING (master)              # Old
! #              LP_OUTGOING_INTEGRATION_UP                     # Old
! #              LP_OUTGOING_INTEGRATION_DN                     # Old
! #                                                             # Old
! #      Version 3.4.  LAP module (column Jacobians)            # Old
! #             LIDORT_LAP_DBCORRECTION                         # Old
! #                                                             # Old
! ############################################################### Old

! ###############################################################
! #                                                             #
! #            SFO_LPS_MASTER_INTERFACE                         #
! #                                                             #
! ###############################################################

!  this is the LPS Linearized interface to the FO code - NEW for LIDORT 3.7 !!!!
...
!   -- 4/9/19. Add FO Surface-leaving assignation and saved cumulative transmittances and linearizations
      ...
      MODULE lidort_sfo_lps_interface_m

      USE SFO_LinMasters_m, Only : SFO_LPS_MASTER

      PUBLIC :: SFO_LPS_MASTER_INTERFACE

      CONTAINS

      SUBROUTINE SFO_LPS_MASTER_INTERFACE ( &
        ...
        DO_PHASFUNC, DO_BRDF_SURFACE, DO_SURFACE_LEAVING, DO_WATER_LEAVING, DO_SL_ISOTROPIC,& ! Input Optical/Surface flags
        DO_PROFILE_LINEARIZATION, DO_SURFACE_LINEARIZATION, DO_SLEAVE_WFS,                  & ! Input Jacobian flags
        NLAYERS, NFINELAYERS, NLEGEN_MOMENTS_INPUT, NBEAMS, BEAM_SZAS,                      & ! Input numbers
        N_USER_STREAMS, USER_VZANGLES, N_USER_RELAZMS, USER_RELAZMS,                        & ! Input geometry
        ...
        CUMTRANS, SLTERM, LP_CUMTRANS, LSSL_SLTERM,                              & ! Output Auxiliary 4/9/19 New line
...
!  Solar
      ...
      Real(fpk), INTENT (INOUT) :: FO_PROFILEWF_SS ( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAX_GEOMETRIES, MAX_DIRECTIONS )
      Real(fpk), INTENT (INOUT) :: FO_PROFILEWF_DB ( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAX_GEOMETRIES )
      Real(fpk), INTENT (INOUT) :: FO_SURFACEWF_DB ( MAX_SURFACEWFS, MAX_USER_LEVELS, MAX_GEOMETRIES )

!  Thermal

      Real(fpk), INTENT(INOUT) :: FO_PROFILEWF_DTA ( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAX_GEOMETRIES, MAX_DIRECTIONS )
      Real(fpk), INTENT(INOUT) :: FO_PROFILEWF_DTS ( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAX_GEOMETRIES )
      Real(fpk), INTENT(INOUT) :: FO_SURFACEWF_DTS ( MAX_SURFACEWFS, MAX_USER_LEVELS, MAX_GEOMETRIES )

!  Composite

      Real(fpk), INTENT(INOUT) :: FO_PROFILEWF_ATMOS ( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAX_GEOMETRIES, MAX_DIRECTIONS )
      Real(fpk), INTENT(INOUT) :: FO_PROFILEWF_SURF  ( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAX_GEOMETRIES )
      Real(fpk), INTENT(INOUT) :: FO_PROFILEWF       ( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAX_GEOMETRIES, MAX_DIRECTIONS )
...
!  4/9/19. Additional linearized output for the sleave correction
      ...
      real(fpk), Intent(out) :: LP_CUMTRANS ( max_user_levels, max_geometries, maxlayers, MAX_ATMOSWFS)
...
!  Other variables
      ...
      integer   :: ns, nv, na, nv_offset(maxbeams), na_offset(maxbeams,max_user_streams)
      INTEGER   :: G, L, N, PAR, SPAR, UT
      INTEGER   :: LUM=1, LUA=1
...
!  BRDF reflection now also for Lattice.
      ...
      if (do_lambertian) then
        ...
        if ( do_ObsGeom ) then
           do g=1,ngeoms
              reflec(g) = EXACTDB_BRDFUNC(lum,lua,g)
           end do
        else
          do ns = 1, nszas
            do nv = 1, nvzas
...
!  Version 3.8 (FO 1.5). Optional use of Phasfunction input
      ...
      if (do_surfacewfs) then
        if (do_lambertian) then
          ...
          do spar=1,n_reflecwfs
            if ( do_ObsGeom ) then
              do g=1,ngeoms
                 LS_reflec(g,spar) = LS_EXACTDB_BRDFUNC(SPAR,LUM,LUA,G)
              end do
            else
              do ns = 1, nszas
                do nv = 1, nvzas
...
!mick fix 3/22/2017 - added "do_surface_emission" to if condition
      ...
      if ( do_surface_emission .and. do_surfacewfs ) then
        do spar=1,n_reflecwfs
          if ( do_ObsGeom ) then
            do g=1,ngeoms
              LS_emiss(g,spar) = LS_USER_EMISSIVITY(spar,g)
            end do
          else
            do nv=1,nvzas
              LS_emiss(nv,spar) = LS_USER_EMISSIVITY(spar,nv)
            end do
          endif
        end do
      end if
...
!   4/9/19. CUMTRANS and LP_CUMTRANS now additional output. add water-leaving control.
      ...
      CALL SFO_LPS_MASTER &
       ( maxgeoms, maxszas, maxvzas, maxazms, maxlayers, max_partlayers, maxfine, maxmoments_input, & ! Input max dims
         ...
         do_upwelling, do_dnwelling, do_deltam_scaling, do_phasfunc, do_obsgeom,                    & ! Input flags (general)
         ...
         ngeoms, nszas, nvzas, nazms, nlayers, nfine, nmoments_input,                               & ! Input Numbers
         ...
         fo_intensity_ss,  fo_intensity_db,  fo_intensity_dta, fo_intensity_dts,                    & ! Output - Intensity
         fo_profilewf_ss,  fo_profilewf_db,  fo_profilewf_dta, fo_profilewf_dts,                    & ! Output - Profile Jacobians
         fo_surfacewf_db,  fo_surfacewf_dts,                                                        & ! Output - Profile Jacobians
         fo_intensity_atmos,  fo_intensity_surf, fo_intensity,                                      & ! Output - Intensity composite
         fo_profilewf_atmos,  fo_profilewf_surf, fo_profilewf, fo_surfacewf,                        & ! Output - Jacobians composite
         Cumtrans, LP_cumtrans, fail, message, trace_1, trace_2 )                                     ! Output (Exception handling)

!  Done

      RETURN
END SUBROUTINE SFO_LPS_MASTER_INTERFACE

!  finish module

END MODULE lidort_sfo_lps_interface_m
