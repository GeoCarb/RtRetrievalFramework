...
!  Downwelling multipliers
            ...
            if ( ABS(GAMMA_M(AA,N)) .LT. TAYLOR_SMALL ) THEN
               ...
               MULTDN = PMULT_UD(AA,UM,N) / ATERM_SAVE(AA,N) ; IGAM = ONE / GAMMA_M(AA,N)
               DO Q = 1, K_PARAMETERS
...
!  Upwelling multipliers
            ...
            MULTUP = PMULT_UU(AA,UM,N) / BTERM_SAVE(AA,N) ; IGAM = ONE / GAMMA_P(AA,N)
            DO Q = 1, K_PARAMETERS
               L_GAMMA  = LC_AVERAGE_SECANT(N,IB,Q) + L_KEIGEN(AA,N,Q)
...
!  Add the particular solution contributions
            ...
            DO Q = 1, K_PARAMETERS
               H1 = U_XPOS(UM,AA,N) * L_PMULT_DN(Q) + L_U_XPOS(UM,AA,N,Q) * PMULT_UD(AA,UM,N)
               H2 = U_XNEG(UM,AA,N) * L_PMULT_UP(Q) + L_U_XNEG(UM,AA,N,Q) * PMULT_UU(AA,UM,N)
               SPAR(Q) = SPAR(Q) + H1 + H2
            ENDDO
...
!  Downwelling multipliers. Note use of -YFAC in L_2b. Rob 01/09/14
            ...
            if ( ABS(GAMMA_M(AA,N)) .LT. TAYLOR_SMALL ) THEN
               ...
               MULTDN = PMULT_DD(AA,UM,N) / ATERM_SAVE(AA,N) ; IGAM = ONE / GAMMA_M(AA,N)
               DO Q = 1, K_PARAMETERS
                  L_GAMMA  = LC_AVERAGE_SECANT(N,IB,Q) - L_KEIGEN(AA,N,Q)
...
!  Upwelling multipliers
            ...
            MULTUP = PMULT_DU(AA,UM,N) / BTERM_SAVE(AA,N) ; IGAM = ONE / GAMMA_P(AA,N)
            DO Q = 1, K_PARAMETERS
               L_GAMMA  = LC_AVERAGE_SECANT(N,IB,Q) + L_KEIGEN(AA,N,Q)
...
!  Add the particular solution contributions
            ...
            DO Q = 1, K_PARAMETERS
               H1 = U_XNEG(UM,AA,N) * L_PMULT_DN(Q) + L_U_XNEG(UM,AA,N,Q) * PMULT_DD(AA,UM,N)
               H2 = U_XPOS(UM,AA,N) * L_PMULT_UP(Q) + L_U_XPOS(UM,AA,N,Q) * PMULT_DU(AA,UM,N)
               SPAR(Q) = SPAR(Q) + H1 + H2
            ENDDO
...
!   * Use of L_2b Taylor series applied twice with UXUP was critical. 01/09/14
            ...
            if ( ABS(GAMMA_M(AA,N)) .LT. TAYLOR_SMALL ) THEN
               ...
               MULTDN = UT_PMULT_UD(AA,UM,UT) / ATERM_SAVE(AA,N) ; IGAM = ONE / GAMMA_M(AA,N)
               DO Q = 1, K_PARAMETERS
                  L_GAMMA  = LC_AVERAGE_SECANT(N,IB,Q) - L_KEIGEN(AA,N,Q)
...
!  Upwelling multipliers
            ...
            MULTUP = UT_PMULT_UU(AA,UM,UT) / BTERM_SAVE(AA,N) ; IGAM = ONE / GAMMA_P(AA,N)
            DO Q = 1, K_PARAMETERS
               L_GAMMA  = LC_AVERAGE_SECANT(N,IB,Q) + L_KEIGEN(AA,N,Q)
...
!  Add the particular solution contributions
            ...
            DO Q = 1, K_PARAMETERS
               H1 = U_XPOS(UM,AA,N) * L_PMULT_DN(Q) + L_U_XPOS(UM,AA,N,Q) * UT_PMULT_UD(AA,UM,UT)
               H2 = U_XNEG(UM,AA,N) * L_PMULT_UP(Q) + L_U_XNEG(UM,AA,N,Q) * UT_PMULT_UU(AA,UM,UT)
               SPAR(Q) = SPAR(Q) + H1 + H2
            ENDDO
...
!  Downwelling multipliers. Note use of -YFAC in the L_2b Call. 01/09/14. Rob checked.
            ...
            if ( ABS(GAMMA_M(AA,N)) .LT. TAYLOR_SMALL ) THEN
               ...
               MULTDN = UT_PMULT_DD(AA,UM,UT)  / ATERM_SAVE(AA,N) ; IGAM = ONE / GAMMA_M(AA,N)
               DO Q = 1, K_PARAMETERS
                  L_GAMMA  = LC_AVERAGE_SECANT(N,IB,Q) - L_KEIGEN(AA,N,Q)
...
!  Upwelling multipliers
            ...
            MULTUP = UT_PMULT_DU(AA,UM,UT) / BTERM_SAVE(AA,N) ; IGAM = ONE / GAMMA_P(AA,N)
            DO Q = 1, K_PARAMETERS
               L_GAMMA  = LC_AVERAGE_SECANT(N,IB,Q) + L_KEIGEN(AA,N,Q)
...
!  Add the particular solution contributions
            ...
            DO Q = 1, K_PARAMETERS
               H1 = U_XNEG(UM,AA,N) * L_PMULT_DN(Q) + L_U_XNEG(UM,AA,N,Q) * UT_PMULT_DD(AA,UM,UT)
               H2 = U_XPOS(UM,AA,N) * L_PMULT_UP(Q) + L_U_XPOS(UM,AA,N,Q) * UT_PMULT_DU(AA,UM,UT)
               SPAR(Q) = SPAR(Q) + H1 + H2
            ENDDO
...
!  Complete the multipliers
         ...
         ITA = ITRANS * ATERM_SAVE(AA,N); GDN = UT_GMULT_DN(AA,UT)
         ITB = ITRANS * BTERM_SAVE(AA,N); GUP = UT_GMULT_UP(AA,UT)
         DO Q = 1, K_PARAMETERS
            LC_UT_GMULT_DN(AA,UT,Q) = ITA * L_MULTDN(Q) + GDN * ( L_ITRANS(Q) + L_ATERM_SAVE(AA,N,Q) )
            LC_UT_GMULT_UP(AA,UT,Q) = ITB * L_MULTUP(Q) + GUP * ( L_ITRANS(Q) + L_BTERM_SAVE(AA,N,Q) )
         ENDDO
...
!  Sum up the Green's function contributions
      ...
      DO I = 1, NSTREAMS
        ...
        DO Q = 1, K_PARAMETERS
          SPAR = ZERO
          DO AA = 1, NSTREAMS
            PAR1 = L_XPOS(I,AA,N,Q)  * UT_GMULT_UP(AA,UT) + XPOS(I,AA,N)  * LC_UT_GMULT_UP(AA,UT,Q)
            PAR2 = L_XPOS(I1,AA,N,Q) * UT_GMULT_DN(AA,UT) + XPOS(I1,AA,N) * LC_UT_GMULT_DN(AA,UT,Q)
            SPAR = SPAR + PAR1 + PAR2
          ENDDO
          QUADCOLUMNWF(Q,UTA,I,IB,UPIDX) = QUADCOLUMNWF(Q,UTA,I,IB,UPIDX) + FMULT * SPAR
        ENDDO
