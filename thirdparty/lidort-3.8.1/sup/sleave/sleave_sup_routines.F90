...
!    -- Introduced the Rough_Surface parameter into main argument list.
      ...
      PRIVATE:: WhiteCap_Reflectance, Water_Transmittance, &
                Interpolate_fOQ_BS1, Interpolate_fOQ_BS2,  &
                Ocean_Reflectance_Basic, GENERAL_SUNGLINT

      PUBLIC :: WaterLeaving_2,            &
                Water_RefracIndex,         &
                Water_Transmittance_Quads, &
                Fresnel_Complex,           &
                get_fluorescence_755,      &
...
!  Help
   ...
   real(fpk) :: incident_angle, Local_Sine, SZA_cosines(Maxszas)
...
!   REAL(fpk) :: WLeaving_ISO_Alt ( Maxszas )
...
!      WLeaving_Iso(J) = Const_Iso * Ocean_Iso_Reflecs(J) ! Old, pre-patch
...
!    -- Must use directional Ocean reflectance terms for the NON-ISOTROPY case
      ...
      if ( do_isotropy ) then
         ...
         do I = 1, nstreams
!            WLeaving_SD(J,I) = Const_Iso * Trans_Stream(I) * Ocean_SD_Reflecs(J,I) ! Old, pre-patch
            WLeaving_SD(J,I) = Const_Iso * Trans_Stream(I) * Ocean_SD_Reflecs(J,I)*SZA_cosines(J)
         enddo
         do I = 1, nvzas
!            WLeaving_SV(J,I) = Const_Iso * Trans_Viewing(I) * Ocean_SV_Reflecs(J,I) ! Old, pre-patch
            WLeaving_SV(J,I) = Const_Iso * Trans_Viewing(I) * Ocean_SV_Reflecs(J,I)*SZA_cosines(J)
         enddo
      endif
...
! R Spurr 01-05 October 2015
      ...
      if ( Do_Approximate_Ta ) then
         ...
         Call TaSav_Interpolate &
            ( nTa_szas, nTa_wavs, Ta_szas, Ta_wavs, TaSavData, Wavelength, SZA_cosines(J), Ta)
      endif
...
!  Debug output
!      tau_rayleigh=0.008569*(Wavelength**(-4))*(1+0.0113*(Wavelength**(-2)) +0.00013*(Wavelength**(-4)))
!      write(776,'(f8.3,1p10e11.4)')Wavelength*1000.0d0, TaSav(1:nszas,1),exp(-0.5*tau_rayleigh/SZA_cosines(1))

! Normal return

   return

!mick fix 3/22/2017 - added error return section
!  Error return

88 continue
   fail = .true.
   message = 'Openfile error in WaterLeaving_2; file not found: ' // Trim(TaRayFile)

end subroutine WaterLeaving_2
...
!  input is double precision from the main routine
   ...
   real(fpk)    , intent(in)   :: Ta_szas   (14)
   real(fpk)    , intent(in)   :: Ta_wavs   (64)
   real(fpk)    , intent(in)   :: TaSavData (64,14,4)
...
!  start
   ...
   do j = 1, nTa_szas
      j1 = 1 + nTa_szas - j
      cosTa_szas(j1) = cos(Ta_szas(j)*dtr)
   enddo
