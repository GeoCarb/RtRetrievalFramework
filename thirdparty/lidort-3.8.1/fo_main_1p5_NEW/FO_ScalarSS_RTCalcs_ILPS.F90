...
!  Source function integration results
   ...
   real(ffp)  :: lostrans_up (maxlayers), lostrans_up_p (maxpartials)
   real(ffp)  :: LP_sources_up     ( maxlayers,maxlayers,max_atmoswfs )
   real(ffp)  :: LP_lostrans_up    ( maxlayers,max_atmoswfs )
...
!mick fix 3/22/2017 - replaced index "np" with "n" in "LosW_paths"
            ...
            lostau = kn * path_up ; if( lostau.lt.cutoff ) lostrans_up(n) = exp ( - lostau )
            if ( do_profilewfs ) then
              if ( Qvary(n) ) then
                do q = 1, Qnums(n)
                ...
                if ( k.eq.n ) then
                  do q = 1, Qnums(k)
                    L_cumsource(q) = LP_SOURCES_UP(N,K,Q) + &
                           LP_LOSTRANS_UP(N,Q) * CUMSOURCE_UP(NC-1) + LOSTRANS_UP(N) * L_CUMSOURCE(Q)
                  enddo
                else
                  do q = 1, Qnums(k)
...
!mick fix 3/22/2017 - initialize NC
            ...
            DO UTA = N_USER_LEVELS, 1, -1
              ...
              DO N = NSTART, NUT, -1
                ...
                if ( k.eq.n ) then
                  do q = 1, Qnums(k)
                    L_cumsource(q) =  LP_LOSTRANS_UP(N,Q) * CUMSOURCE_DB(NC-1) + &
...
!  5/5/20. Version 3.8.1 Upgrades. CUMTRANS calculation was not properly initialized (no Jacobians)
...
!  Source function integration results
   ...
   real(ffp)  :: lostrans_dn (maxlayers), lostrans_dn_p (maxpartials)
   real(ffp)  :: LP_sources_dn     ( maxlayers,maxlayers,max_atmoswfs )
   real(ffp)  :: LP_lostrans_dn    ( maxlayers,max_atmoswfs )
                ...
                if ( k.eq.n ) then
                  do q = 1, Qnums(k)
                    L_cumsource(q) = LP_SOURCES_DN(N,K,Q)         + &
                           LP_LOSTRANS_DN(N,Q) * CUMSOURCE_DN(NC-1) + LOSTRANS_DN(N) * L_CUMSOURCE(Q)
                  enddo
                else
                  do q = 1, Qnums(k)
                    L_cumsource(q) = LP_SOURCES_DN(N,K,Q) + LOSTRANS_DN(N) * L_CUMSOURCE(Q)
                  enddo
                endif
              ENDDO
              IF ( Partial_OUTFLAG(UTA) ) THEN
...
subroutine SS_Integral_ILPS_UPDN &
   ( maxgeoms, maxlayers, maxpartials, maxfine, maxmoments_input,                      & ! Inputs (dimensioning)
     ...
     intensity_dn, LP_Jacobians_dn, cumtrans, LP_cumtrans )                                    ! Output (cumtrans and downwelling)
