...
!   real(ffp)              :: cutoff
   ...
   character*2            :: c2
   character*3            :: c3, c33
   integer                :: v, n1, ns, nv, nv_offset(maxszas),na_offset(maxszas,maxvzas)
...
!  Mode bookkeeping
   ...
   cmode = 'OBSGEOM' ; if ( .not. do_ObsGeom ) cmode = 'LATTICE'
...
!  End Obsgeom vs. Lattice, Checking
...
!mick fix 3/25/2015 - adjusted extent of 1st dim
      ...
      do v = 1, nvzas
         ...
         if (n1 .gt. maxfine ) then
            ...
            trace   = 'Dimensioning check AFTER call to LosOut_EnhancedPS_FindCrit in SSWPgeometry_master (LATTICE mode)'
            fail = .true. ;  return
          endif
      enddo
