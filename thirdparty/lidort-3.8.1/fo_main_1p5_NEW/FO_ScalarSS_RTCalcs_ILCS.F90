...
!  Source function integration results
   ...
   real(ffp)  :: LC_lostrans_up (maxlayers,max_atmoswfs), LC_lostrans_up_p (maxpartials,max_atmoswfs)
...
! mick mod 3/22/2017 - turned off (not needed)
      ...
      !LC_lostrans_up = zero   ; LC_sources_up = zero   ; L_exactscat_up = zero
            ...
            DO N = NSTART, NUT, -1
               ...
               do q = 1, n_columnwfs
                  L_cumsource(q) = LC_SOURCES_UP(N,Q) + &
                                   LC_LOSTRANS_UP(N,Q) * CUMSOURCE_UP(NC-1) + LOSTRANS_UP(N) * L_CUMSOURCE(Q)
               enddo
            ENDDO
            IF ( Partial_OUTFLAG(UTA) ) THEN
              UT = Partial_OUTINDEX(UTA)
...
!mick fix 3/22/2017 - initialize NC
         ...
         DO UTA = N_USER_LEVELS, 1, -1
            ...
            DO N = NSTART, NUT, -1
               NC = NLAYERS + 1 - N
               do q = 1, n_columnwfs
                 L_cumsource(q) =  LC_LOSTRANS_UP(N,Q) * CUMSOURCE_DB(NC-1) + &
...
!  Source function integration results
   ...
   real(ffp)  :: LC_lostrans_dn (maxlayers,max_atmoswfs), LC_lostrans_dn_p (maxpartials,max_atmoswfs)
          ...
          DO N = NSTART, NUT
            ...
            do q = 1, n_columnwfs
              L_cumsource(q) = LC_SOURCES_DN(N,Q)         + &
                              LC_LOSTRANS_DN(N,Q) * CUMSOURCE_DN(NC-1) + LOSTRANS_DN(N)   * L_CUMSOURCE(Q)
            enddo
          ENDDO
          IF ( Partial_OUTFLAG(UTA) ) THEN
            UT = Partial_OUTINDEX(UTA)
...
subroutine SS_Integral_ILCS_UPDN &
   ( maxgeoms, maxlayers, maxpartials, maxfine, maxmoments_input,                      & ! Inputs (dimensioning)
     ...
     intensity_dn, LC_Jacobians_dn, cumtrans, LC_cumtrans )                                    ! Output (cumtrans and downwelling)
