...
!                       LIDORT_Outputs  Intent(Out)
      ...
      use LIDORT_PARS_m, only : fpk, MAXSTREAMS, MAX_GEOMETRIES, MAX_DIRECTIONS, MAXMOMENTS, &
...
!  Direct Surface-Leaving term
      ...
      REAL(fpk), dimension (  MAX_USER_STREAMS, &
        MAX_USER_RELAZMS, MAXBEAMS ) :: TS_WLADJUSTED_DIRECT
...
!    Every solar direction, SL-transmitted user streams
      ...
      REAL(fpk), dimension ( 0:MAXMOMENTS, MAXSTREAMS, &
        MAXBEAMS )   :: TS_WLADJUSTED_F_Ords_0
      REAL(fpk), dimension ( 0:MAXMOMENTS, MAX_USER_STREAMS, &
        MAXBEAMS )   :: TS_WLADJUSTED_F_User_0
