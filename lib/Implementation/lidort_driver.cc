  ...
  : nstream_(nstream), nmoment_(nmoment), do_multi_scatt_only_(do_multi_scatt_only), surface_type_(surface_type), pure_nadir_(pure_nadir)
{
  ...
  Array<double, 1>temp;
  temp.resize(1);
  temp = lidort_interface_->lidort_fixin().optical().ts_lambertian_albedo();
  lidort_brdf_driver()->set_lambertian_albedo( temp );
...
/*
fboolean_inputs.ts_do_surface_leaving(false);
fboolean_inputs.ts_do_sl_isotropic(false);

fboolean_inputs.ts_do_water_leaving(false);
fboolean_inputs.ts_do_fluorescence(false);

fboolean_inputs.ts_do_tf_iteration(false);
fboolean_inputs.ts_do_wladjusted_output(false);

fboolean_inputs.ts_do_toa_illumination(false);
fboolean_inputs.ts_do_boa_illumination(false);

fboolean_inputs.ts_do_albtrn_media(false);

fboolean_inputs.ts_do_planetary_problem(false);
*/
